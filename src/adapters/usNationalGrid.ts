/**
 * US National Grid Adapter
 * Queries US National Grid FeatureServer for multiple grid layers
 * Supports point-in-polygon queries for polygon layers and proximity queries (up to 25 miles)
 */

// Import the CORS proxy system from EnrichmentService
import { fetchJSONSmart } from '../services/EnrichmentService';

const BASE_SERVICE_URL = 'https://services2.arcgis.com/FiaPA4ga0iQKduv3/arcgis/rest/services/US_National_Grid_HFL_V/FeatureServer';

export interface USNationalGridInfo {
  gridId: string | null;
...
/**
 * Query US National Grid FeatureServer for a specific layer
 * Supports point-in-polygon queries for polygons and proximity queries for all (up to 25 miles)
 */
export async function getUSNationalGridData(
  layerId: number,
  lat: number,
  lon: number,
  radius?: number
): Promise<USNationalGridInfo[]> {
  try {
    // All layers are polygon layers for US National Grid
    const results: USNationalGridInfo[] = [];
    const processedFeatureIds = new Set<string>();
    
    // Always try point-in-polygon first (for containing features)
    console.log(`🗺️ Querying US National Grid Layer ${layerId} for point-in-polygon at [${lat}, ${lon}]`);
    ...
    console.log(`🔗 US National Grid Layer ${layerId} Point-in-Polygon Query URL: ${pointInPolyUrl.toString()}`);
    ...
    try {
      ...
      if (!pointInPolyData.error && pointInPolyData.features && pointInPolyData.features.length > 0) {
        pointInPolyData.features.forEach((feature: any) => {
          ...
          const featureId = gridId ? gridId.toString() : `${feature.OBJECTID || feature.objectid || Math.random()}`;
        ...
        console.log(`✅ Found ${results.length} US National Grid Layer ${layerId} feature(s) containing the point`);
      }
    } catch (error) {
    ...
    console.log(`🔗 US National Grid Layer ${layerId} Proximity Query URL: ${queryUrl.toString()}`);
    ...
    try {
      ...
      if (!proximityData.error && proximityData.features && proximityData.features.length > 0) {
        ...
        console.log(`✅ Found ${proximityData.features.length} US National Grid Layer ${layerId} feature(s) within ${cappedRadius} miles (${results.length} total including containing)`);
      }
    } catch (error) {
    ...
    console.error(`❌ Error querying US National Grid Layer ${layerId}:`, error);
    return [];
  }
}
