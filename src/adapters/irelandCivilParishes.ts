...
export interface IrelandCivilParishInfo {
  objectId: number;
  guid: string;
  engName: string;
  gleName: string;
  gaeltachtArea: string;
  shapeArea: number;
  shapeLength: number;
...
export async function getIrelandCivilParishesData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: IrelandCivilParishInfo[];
  nearby_features: IrelandCivilParishInfo[];
  _all: IrelandCivilParishInfo[];
}> {
  const baseUrl = 'https://services-eu1.arcgis.com/FH5XCsx8rYXqnjF5/ArcGIS/rest/services/CivilParishes_urbanareas_NationalStatutoryBoundaries_Ungeneralised_2024/FeatureServer/1';
  
  const containing: IrelandCivilParishInfo[] = [];
  const nearby_features: IrelandCivilParishInfo[] = [];
  const _all: IrelandCivilParishInfo[] = [];
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        // Extract Civil Parish information
        const parishInfo: IrelandCivilParishInfo = {
          objectId: attributes.OBJECTID || attributes.ESRI_OID || 0,
          guid: attributes.GUID || '',
          engName: attributes.ENG_NAME_VALUE || '',
          gleName: attributes.GLE_NAME_VALUE || '',
          gaeltachtArea: attributes.GAELTACHT_AREA || '',
        ...
        if (isContaining) {
          parishInfo.distance_miles = 0;
          containing.push(parishInfo);
        } else {
          ...
          if (distance <= radiusMiles) {
            parishInfo.distance_miles = distance;
            nearby_features.push(parishInfo);
          }
        }
        
        _all.push(parishInfo);
      }
    ...
    console.error('Error fetching Ireland Civil Parishes data:', error);
  }
