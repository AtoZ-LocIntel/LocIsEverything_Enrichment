...
async function getContainingFeature(
  ...
  try {
    console.log(`🌳 Querying DE Urban Tree Canopy for containing feature at [${lat}, ${lon}]`);
    ...
    if (data.error) {
      console.error('❌ DE Urban Tree Canopy API Error:', data.error);
      return null;
    }
    ...
    console.error('❌ Error querying DE Urban Tree Canopy:', error);
    return null;
  }
}
