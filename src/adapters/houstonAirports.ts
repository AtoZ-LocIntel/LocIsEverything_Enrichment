/**
 * Houston Airports Adapter
 * Queries Houston Airports polygon feature service
 * Supports point-in-polygon and proximity queries up to 25 miles
 */

const BASE_SERVICE_URL = 'https://services.arcgis.com/NummVBqZSIJKUeVR/arcgis/rest/services/COH_Airports_view/FeatureServer/18';

export interface HoustonAirportInfo {
...
/**
 * Query Houston Airports for point-in-polygon and proximity
 * Supports proximity queries up to 25 miles
 */
export async function getHoustonAirportsData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<HoustonAirportInfo[]> {
  try {
    ...
    const results: HoustonAirportInfo[] = [];
    ...
    try {
      ...
      console.log(`✈️ Querying Houston Airports for point-in-polygon at [${lat}, ${lon}]`);
      console.log(`🔗 Houston Airports Point-in-Polygon Query URL: ${pointInPolyUrl}`);
      ...
      if (!pointInPolyData.error && pointInPolyData.features && pointInPolyData.features.length > 0) {
        pointInPolyData.features.forEach((feature: any) => {
          ...
          if (isContaining) {
            ...
            const airportName = attributes.NAME || attributes.name || attributes.NAME1 || attributes.name1 || 
                               attributes.AIRPORT_NAME || attributes.airport_name || attributes.AirportName || null;
      ...
      console.log(`✅ Found ${results.length} Houston Airport(s) containing the point`);
    } catch (error) {
    ...
    if (maxRadius > 0) {
      try {
        ...
        while (hasMore) {
          ...
          if (resultOffset === 0) {
            console.log(`✈️ Querying Houston Airports for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
          }
          
          const proximityData = await fetchJSONSmart(proximityUrl) as any;
          
          if (proximityData.error) {
            console.error('❌ Houston Airports API Error:', proximityData.error);
            break;
          }
        ...
        console.log(`✅ Fetched ${allFeatures.length} total Houston Airports for proximity`);
    ...
    console.log(`✅ Houston Airports: Found ${results.length} airport(s)`);
    return results;
  } catch (error) {
    console.error('❌ Error querying Houston Airports data:', error);
    throw error;
  }
}
