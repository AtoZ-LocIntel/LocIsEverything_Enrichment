/**
 * UK Cancer Alliances Adapter
 * Queries Cancer Alliances July 2023 Boundaries (England) polygon feature service
 * Supports point-in-polygon and proximity queries up to 50 miles
 *
 * Service: Cancer_Alliances_July_2023_Boundaries_EN_BFC (FeatureServer/0)
...
export interface UKCancerAllianceInfo {
  objectId: number;
  cal23cd: string | null;
  cal23nm: string | null;
  bngE: number | null;
...
export async function getUKCancerAlliancesData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: UKCancerAllianceInfo[];
  nearby_features: UKCancerAllianceInfo[];
  _all: UKCancerAllianceInfo[];
}> {
  const baseUrl = 'https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Cancer_Alliances_July_2023_Boundaries_EN_BFC/FeatureServer/0';
  
  const containing: UKCancerAllianceInfo[] = [];
  const nearby_features: UKCancerAllianceInfo[] = [];
  const _all: UKCancerAllianceInfo[] = [];
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        const info: UKCancerAllianceInfo = {
          objectId: attributes.FID || attributes.OBJECTID || attributes.ESRI_OID || 0,
          cal23cd: attributes.CAL23CD || attributes.cal23cd || null,
          cal23nm: attributes.CAL23NM || attributes.cal23nm || null,
