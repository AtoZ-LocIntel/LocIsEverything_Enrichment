/**
 * USFS System Roads Adapter
 * Queries USFS System Roads linear feature service
 * Supports proximity queries up to 100 miles
 */

const BASE_SERVICE_URL = 'https://apps.fs.usda.gov/arcx/rest/services/EDW/EDW_RoadBasic_01/MapServer/0';

export interface USFSSystemRoadInfo {
...
/**
 * Query USFS System Roads for proximity
 * Supports proximity queries up to 100 miles
 */
export async function getUSFSSystemRoadsData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<USFSSystemRoadInfo[]> {
  try {
    ...
    while (hasMore) {
      ...
      if (resultOffset === 0) {
        console.log(`🛣️ Querying USFS System Roads for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
      }
      
      const proximityData = await fetchJSONSmart(proximityUrl) as any;
      
      if (proximityData.error) {
        console.error('❌ USFS System Roads API Error:', proximityData.error);
        break;
      }
    ...
    console.log(`✅ Fetched ${allFeatures.length} total USFS System Roads for proximity`);
    
    const results: USFSSystemRoadInfo[] = [];
    ...
    allFeatures.forEach((feature: any) => {
      ...
      if (geometry && geometry.paths) {
        ...
        if (distanceMiles <= maxRadius) {
          ...
          const roadType = attributes.ROADTYPE || attributes.RoadType || attributes.roadtype || attributes.TYPE || attributes.Type || attributes.type || null;
    ...
    console.log(`✅ USFS System Roads: Found ${results.length} road(s)`);
    return results;
  } catch (error) {
    console.error('❌ Error querying USFS System Roads data:', error);
    throw error;
  }
}
