...
export async function getCAOilGasWellsData(
  ...
  try {
    if (!radius || radius <= 0) {
      console.log('ℹ️ CA Oil and Gas Wells requires a radius for proximity query');
      return [];
    }
    ...
    console.log(`🛢️ Querying CA Oil and Gas Wells within ${cappedRadius} miles of [${lat}, ${lon}]`);
    ...
    console.log(`🔗 CA Oil and Gas Wells Proximity Query URL: ${queryUrl.toString()}`);
    ...
    if (data.error) {
      console.error('❌ CA Oil and Gas Wells API Error:', data.error);
      return [];
    }
    
    if (!data.features || data.features.length === 0) {
      console.log('ℹ️ No CA Oil and Gas Wells found within the specified radius');
      return [];
    }
    ...
    console.log(`✅ Found ${results.length} CA Oil and Gas Well(s) within ${cappedRadius} miles`);
    return results;
  } catch (error) {
    console.error('❌ Error querying CA Oil and Gas Wells:', error);
    return [];
  }
}
