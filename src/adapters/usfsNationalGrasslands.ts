/**
 * USFS National Grassland Units Adapter
 * Queries USFS National Grassland Units polygon feature service
 * Supports point-in-polygon and proximity queries up to 50 miles
 */

const BASE_SERVICE_URL = 'https://apps.fs.usda.gov/arcx/rest/services/EDW/EDW_NationalGrassland_01/MapServer/0';

export interface USFSNationalGrasslandInfo {
  objectId: string | null;
  grasslandName: string | null;
  grasslandId: string | null;
...
/**
 * Query USFS National Grassland Units for point-in-polygon and proximity
 * Supports proximity queries up to 50 miles
 */
export async function getUSFSNationalGrasslandsData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<USFSNationalGrasslandInfo[]> {
  try {
    ...
    const results: USFSNationalGrasslandInfo[] = [];
    ...
    try {
      ...
      console.log(`🌾 Querying USFS National Grassland Units for point-in-polygon at [${lat}, ${lon}]`);
      console.log(`🔗 USFS National Grasslands Point-in-Polygon Query URL: ${pointInPolyUrl}`);
      ...
      if (!pointInPolyData.error && pointInPolyData.features && pointInPolyData.features.length > 0) {
        pointInPolyData.features.forEach((feature: any) => {
          ...
          if (isContaining) {
            ...
            const grasslandName = attributes.GRASSLANDNAME || attributes.GrasslandName || attributes.grasslandname || attributes.NATIONALGRASSLANDNAME || attributes.NationalGrasslandName || attributes.nationalgrasslandname || null;
            const grasslandId = attributes.NATIONALGRASSLANDID !== null && attributes.NATIONALGRASSLANDID !== undefined ? attributes.NATIONALGRASSLANDID.toString() : null;
            const gisAcres = attributes.GIS_ACRES !== null && attributes.GIS_ACRES !== undefined ? Number(attributes.GIS_ACRES) : null;
            
            results.push({
              objectId: objectId,
              grasslandName: grasslandName,
              grasslandId: grasslandId,
              gisAcres: gisAcres,
              geometry: geometry,
              distance_miles: 0,
              isContaining: true,
      ...
      console.log(`✅ Found ${results.length} USFS National Grassland Unit(s) containing the point`);
    } catch (error) {
    ...
    if (maxRadius > 0) {
      try {
        ...
        while (hasMore) {
          ...
          if (resultOffset === 0) {
            console.log(`🌾 Querying USFS National Grassland Units for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
          }
          
          const proximityData = await fetchJSONSmart(proximityUrl) as any;
          
          if (proximityData.error) {
            console.error('❌ USFS National Grasslands API Error:', proximityData.error);
            break;
          }
        ...
        console.log(`✅ Fetched ${allFeatures.length} total USFS National Grassland Units for proximity`);
    ...
    console.log(`✅ USFS National Grassland Units: Found ${results.length} grassland unit(s)`);
    return results;
  } catch (error) {
    console.error('❌ Error querying USFS National Grassland Units data:', error);
    throw error;
  }
}
