/**
 * DE Outdoor Recreation, Parks and Trails Program Open Space Adapter
...
export interface DEOutdoorRecreationParksTrailsOpenSpaceData {
  containingFeature: DEOutdoorRecreationParksTrailsOpenSpaceInfo | null;
  nearbyFeatures: DEOutdoorRecreationParksTrailsOpenSpaceInfo[];
}

export interface DEOutdoorRecreationParksTrailsOpenSpaceInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

async function getContainingFeature(lat: number, lon: number): Promise<DEOutdoorRecreationParksTrailsOpenSpaceInfo | null> {
  try {
    ...
    console.error('❌ Error querying DE Outdoor Recreation Parks Trails Open Space:', error);
    return null;
  }
}

async function getNearbyFeatures(lat: number, lon: number, radiusMiles: number): Promise<DEOutdoorRecreationParksTrailsOpenSpaceInfo[]> {
  try {
    ...
    const features: DEOutdoorRecreationParksTrailsOpenSpaceInfo[] = data.features.map((feature: any) => {
...
export async function getDEOutdoorRecreationParksTrailsOpenSpaceData(
  lat: number,
  lon: number,
  radiusMiles: number = 0
): Promise<DEOutdoorRecreationParksTrailsOpenSpaceData> {
