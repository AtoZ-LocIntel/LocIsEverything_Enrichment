...
export interface IrelandBuiltUpAreaInfo {
  objectId: number;
  fcSubtype: number;
  fCode: string;
  shapeArea: number;
...
export async function getIrelandBuiltUpAreasData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: IrelandBuiltUpAreaInfo[];
  nearby_features: IrelandBuiltUpAreaInfo[];
  _all: IrelandBuiltUpAreaInfo[];
}> {
  const baseUrl = 'https://services-eu1.arcgis.com/FH5XCsx8rYXqnjF5/ArcGIS/rest/services/Built_Up_Areas___OSi_National_1m_Map_Of_Ireland/FeatureServer/0';
  
  const containing: IrelandBuiltUpAreaInfo[] = [];
  const nearby_features: IrelandBuiltUpAreaInfo[] = [];
  const _all: IrelandBuiltUpAreaInfo[] = [];
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        // Extract built-up area information
        const builtUpAreaInfo: IrelandBuiltUpAreaInfo = {
          objectId: attributes.OBJECTID || 0,
          fcSubtype: attributes.FCsubtype || 0,
        ...
        if (isContaining) {
          builtUpAreaInfo.distance_miles = 0;
          containing.push(builtUpAreaInfo);
        } else {
          ...
          if (distance <= radiusMiles) {
            builtUpAreaInfo.distance_miles = distance;
            nearby_features.push(builtUpAreaInfo);
          }
        }
        
        _all.push(builtUpAreaInfo);
      }
