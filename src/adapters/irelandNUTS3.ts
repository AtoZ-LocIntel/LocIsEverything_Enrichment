...
export interface IrelandNUTS3Info {
  objectId: number;
  nuts1: string;
  nuts1Name: string;
  nuts2: string;
...
export async function getIrelandNUTS3Data(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: IrelandNUTS3Info[];
  nearby_features: IrelandNUTS3Info[];
  _all: IrelandNUTS3Info[];
}> {
  const baseUrl = 'https://services-eu1.arcgis.com/FH5XCsx8rYXqnjF5/ArcGIS/rest/services/NUTS3_Boundaries_Ungeneralised/FeatureServer/0';
  
  const containing: IrelandNUTS3Info[] = [];
  const nearby_features: IrelandNUTS3Info[] = [];
  const _all: IrelandNUTS3Info[] = [];
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        // Extract NUTS3 information
        const nuts3Info: IrelandNUTS3Info = {
        ...
        if (isContaining) {
          nuts3Info.distance_miles = 0;
          containing.push(nuts3Info);
        } else {
          ...
          if (distance <= radiusMiles) {
            nuts3Info.distance_miles = distance;
            nearby_features.push(nuts3Info);
          }
        }
    ...
    console.error('Error fetching Ireland NUTS3 Boundaries data:', error);
  }
