/**
 * DE Pine Plantations Adapter
 * Queries Delaware Pine Plantations from DE FirstMap FeatureServer
...
export interface DEPinePlantationData {
  containingFeature: DEPinePlantationInfo | null;
  nearbyFeatures: DEPinePlantationInfo[];
}

export interface DEPinePlantationInfo {
...
async function getContainingFeature(
  lat: number,
  lon: number
): Promise<DEPinePlantationInfo | null> {
  try {
    console.log(`🌲 Querying DE Pine Plantations for containing feature at [${lat}, ${lon}]`);
    ...
    if (data.error) {
      console.error('❌ DE Pine Plantations API Error:', data.error);
      return null;
    }
    ...
    console.error('❌ Error querying DE Pine Plantations:', error);
    return null;
  }
}
...
async function getNearbyFeatures(
  ...
  try {
    ...
    const features: DEPinePlantationInfo[] = data.features.map((feature: any) => {
...
export async function getDEPinePlantationData(
  lat: number,
  lon: number,
  radiusMiles: number = 0
): Promise<DEPinePlantationData> {
