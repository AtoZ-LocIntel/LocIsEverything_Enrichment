/**
 * TIGER Native Lands Adapter
 * Queries US Census TIGER American Indian, Alaska Native, and Native Hawaiian Areas MapServer
 * Supports point-in-polygon and proximity queries up to 25 miles
 * API: https://tigerweb.geo.census.gov/arcgis/rest/services/TIGERweb/AIANNHA/MapServer
 */

import { fetchJSONSmart } from '../services/EnrichmentService';

const BASE_SERVICE_URL = 'https://tigerweb.geo.census.gov/arcgis/rest/services/TIGERweb/AIANNHA/MapServer';

export interface TIGERNativeLandInfo {
  objectId: number;
  name?: string | null;
...
/**
 * Query TIGER Native Lands MapServer for a specific layer
 * Supports both point-in-polygon and proximity queries
 */
export async function getTIGERNativeLandsData(
  layerId: number,
  lat: number,
  lon: number,
  radius?: number
): Promise<{ containing: TIGERNativeLandInfo | null; nearby: TIGERNativeLandInfo[] }> {
  try {
    const results: { containing: TIGERNativeLandInfo | null; nearby: TIGERNativeLandInfo[] } = {
      containing: null,
      nearby: []
    };
    ...
    try {
      ...
      console.log(`🏛️ Querying TIGER Native Lands Layer ${layerId} for point-in-polygon at [${lat}, ${lon}]`);
      ...
      if (!pointInPolyData.error && pointInPolyData.features && pointInPolyData.features.length > 0) {
        ...
        if (geom && geom.rings) {
          results.containing = {
            ...
            name: attrs.NAME || attrs.name || attrs.NAMELSAD || attrs.NAMELSAD || attrs.FULLNAME || attrs.fullName || attrs.AIANNHCE || attrs.aiannhce || null,
            ...
            areaType: attrs.AIANNHCE || attrs.aiannhce || attrs.TYPE || attrs.type || null,
            geometry: geom,
            distance_miles: 0, // Containing polygon has distance 0
            attributes: attrs
          };
          console.log(`✅ Found containing native land area: ${results.containing.name || 'Unknown'}`);
        }
      }
    } catch (error) {
    ...
    if (maxRadius > 0) {
      try {
        ...
        while (hasMore) {
          ...
          if (resultOffset === 0) {
            console.log(`🏛️ Querying TIGER Native Lands Layer ${layerId} for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
          }
          
          const proximityData = await fetchJSONSmart(proximityUrl.toString());
          
          if (proximityData.error) {
            console.error(`❌ TIGER Native Lands Layer ${layerId} API Error:`, proximityData.error);
            break;
          }
        ...
        console.log(`✅ Fetched ${allFeatures.length} total TIGER Native Lands for proximity`);
        ...
        console.log(`✅ TIGER Native Lands Layer ${layerId}: Found ${results.nearby.length} nearby areas`);
      } catch (error) {
    ...
    console.error(`❌ Error in getTIGERNativeLandsData for layer ${layerId}:`, error);
