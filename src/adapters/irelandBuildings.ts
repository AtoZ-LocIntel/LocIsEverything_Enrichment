...
export interface IrelandBuildingInfo {
  objectId: number;
  buildingType: string;
...
export async function getIrelandBuildingsData(
  lat: number,
  lon: number,
  radiusMiles: number,
  layerId: number,
  ...
  containing: IrelandBuildingInfo[];
  nearby_features: IrelandBuildingInfo[];
  _all: IrelandBuildingInfo[];
}> {
  const baseUrl = `https://services-eu1.arcgis.com/FH5XCsx8rYXqnjF5/ArcGIS/rest/services/Buildings_OpenDatatest3view/FeatureServer/${layerId}`;
  
  const containing: IrelandBuildingInfo[] = [];
  const nearby_features: IrelandBuildingInfo[] = [];
  const _all: IrelandBuildingInfo[] = [];
  ...
  try {
    ...
    const queryUrl = `${baseUrl}/query?f=json&where=1=1&outFields=*&geometry=${encodeURIComponent(JSON.stringify(bufferGeometry))}&geometryType=esriGeometryPoint&spatialRel=esriSpatialRelIntersects&distance=${radiusMeters}&units=esriSRUnit_Meter&inSR=4326&outSR=4326&returnGeometry=true&resultRecordCount=1000`;
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        const buildingInfo: IrelandBuildingInfo = {
        ...
        if (isContaining) {
          buildingInfo.distance_miles = 0;
          containing.push(buildingInfo);
        } else {
          ...
          if (distance <= radiusMiles) {
            buildingInfo.distance_miles = distance;
            nearby_features.push(buildingInfo);
          }
        }
        
        _all.push(buildingInfo);
      }
    ...
    console.error(`Error fetching Ireland Buildings (${buildingType}) data:`, error);
  }
