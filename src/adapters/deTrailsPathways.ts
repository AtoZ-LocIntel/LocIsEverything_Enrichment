/**
 * DE Trails and Pathways Adapter
...
export interface DETrailPathwayInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

export async function getDETrailsPathwaysData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<DETrailPathwayInfo[]> {
  try {
    ...
    const features: DETrailPathwayInfo[] = data.features.map((feature: any) => {
    ...
    console.error('❌ Error querying DE Trails and Pathways:', error);
    return [];
  }
}
