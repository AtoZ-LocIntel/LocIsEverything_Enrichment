/**
 * CA Sandhill Crane Range Adapter
 * Queries California Sandhill Crane Range from CDFW BIOS FeatureServer
 * Supports point-in-polygon and proximity queries
 */

const BASE_SERVICE_URL = 'https://services2.arcgis.com/Uq9r85Potqm3MfRV/arcgis/rest/services/biosds1459_fpu/FeatureServer';
const LAYER_ID = 0;

export interface CASandhillCraneRangeInfo {
  rangeId: string | null;
...
/**
 * Query CA Sandhill Crane Range FeatureServer for point-in-polygon and proximity
 */
export async function getCASandhillCraneRangeData(
  lat: number,
  lon: number,
  radius?: number
): Promise<CASandhillCraneRangeInfo[]> {
  try {
    const results: CASandhillCraneRangeInfo[] = [];
    
    // Point-in-polygon query
    console.log(`🦩 Querying CA Sandhill Crane Range for containing range at [${lat}, ${lon}]`);
    ...
    if (radius && radius > 0) {
      console.log(`🦩 Querying CA Sandhill Crane Range within ${radius} miles of [${lat}, ${lon}]`);
    ...
    console.log(`✅ Found ${results.length} CA Sandhill Crane Range/Ranges`);
    return results;
  } catch (error) {
    console.error('❌ Error querying CA Sandhill Crane Range:', error);
    return [];
  }
}
