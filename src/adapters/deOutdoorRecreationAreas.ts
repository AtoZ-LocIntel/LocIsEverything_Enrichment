/**
 * DE Outdoor Recreation Areas Adapter
...
export interface DEOutdoorRecreationAreaData {
  containingFeature: DEOutdoorRecreationAreaInfo | null;
  nearbyFeatures: DEOutdoorRecreationAreaInfo[];
}

export interface DEOutdoorRecreationAreaInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

async function getContainingFeature(lat: number, lon: number): Promise<DEOutdoorRecreationAreaInfo | null> {
  try {
    ...
    console.error('❌ Error querying DE Outdoor Recreation Areas:', error);
    return null;
  }
}

async function getNearbyFeatures(lat: number, lon: number, radiusMiles: number): Promise<DEOutdoorRecreationAreaInfo[]> {
  try {
    ...
    const features: DEOutdoorRecreationAreaInfo[] = data.features.map((feature: any) => {
...
export async function getDEOutdoorRecreationAreasData(
  lat: number,
  lon: number,
  radiusMiles: number = 0
): Promise<DEOutdoorRecreationAreaData> {
