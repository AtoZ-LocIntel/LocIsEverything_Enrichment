/**
 * DE Outdoor Recreation, Parks and Trails Program Lands Adapter
...
export interface DEOutdoorRecreationParksTrailsLandsData {
  containingFeature: DEOutdoorRecreationParksTrailsLandsInfo | null;
  nearbyFeatures: DEOutdoorRecreationParksTrailsLandsInfo[];
}

export interface DEOutdoorRecreationParksTrailsLandsInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

async function getContainingFeature(lat: number, lon: number): Promise<DEOutdoorRecreationParksTrailsLandsInfo | null> {
  try {
    ...
    console.error('❌ Error querying DE Outdoor Recreation Parks Trails Lands:', error);
    return null;
  }
}

async function getNearbyFeatures(lat: number, lon: number, radiusMiles: number): Promise<DEOutdoorRecreationParksTrailsLandsInfo[]> {
  try {
    ...
    const features: DEOutdoorRecreationParksTrailsLandsInfo[] = data.features.map((feature: any) => {
...
export async function getDEOutdoorRecreationParksTrailsLandsData(
  lat: number,
  lon: number,
  radiusMiles: number = 0
): Promise<DEOutdoorRecreationParksTrailsLandsData> {
