/**
 * BLM National GTLF Limited Public Motorized Roads Adapter
 * Queries BLM National GTLF Limited Public Motorized Roads linear feature service
 * Supports proximity queries up to 50 miles
 */

const BASE_SERVICE_URL = 'https://services1.arcgis.com/KbxwQRRfWyEYLgp4/arcgis/rest/services/BLM_Natl_GTLF_Limited_Public_Motorized_Roads/FeatureServer/0';

export interface BLMNationalLimitedMotorizedRoadInfo {
...
/**
 * Query BLM National GTLF Limited Public Motorized Roads for proximity
 * Supports proximity queries up to 50 miles
 */
export async function getBLMNationalLimitedMotorizedRoadsData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<BLMNationalLimitedMotorizedRoadInfo[]> {
  try {
    ...
    while (hasMore) {
      ...
      if (resultOffset === 0) {
        console.log(`🟠 Querying BLM National GTLF Limited Public Motorized Roads for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
      }
      
      const proximityData = await fetchJSONSmart(proximityUrl) as any;
      
      if (proximityData.error) {
        console.error('❌ BLM Limited Motorized Roads API Error:', proximityData.error);
        break;
      }
    ...
    console.log(`✅ Fetched ${allFeatures.length} total BLM Limited Motorized Roads for proximity`);
    
    const results: BLMNationalLimitedMotorizedRoadInfo[] = [];
    ...
    console.log(`✅ BLM National GTLF Limited Public Motorized Roads: Found ${results.length} road(s)`);
    return results;
  } catch (error) {
    console.error('❌ Error querying BLM National GTLF Limited Public Motorized Roads data:', error);
    throw error;
  }
}
