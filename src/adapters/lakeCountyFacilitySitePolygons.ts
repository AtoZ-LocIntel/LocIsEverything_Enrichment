...
const BASE_SERVICE_URL = 'https://services3.arcgis.com/HESxeTbDliKKvec2/arcgis/rest/services/LakeCounty_Landmarks/FeatureServer/1';

export interface LakeCountyFacilitySitePolygonInfo {
  objectId: number;
  facilityId?: string;
  name: string;
  fcode: string;
...
/**
 * Query Lake County Facility Site Polygons for point-in-polygon and proximity
 * Supports proximity queries up to 5 miles
 */
export async function getLakeCountyFacilitySitePolygonsData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<LakeCountyFacilitySitePolygonInfo[]> {
  try {
    // Cap radius at 5 miles
    const maxRadius = radiusMiles ? Math.min(radiusMiles, 5.0) : 5.0;
    
    const results: LakeCountyFacilitySitePolygonInfo[] = [];
    ...
    try {
      ...
      console.log(`🏛️ Querying Lake County Facility Site Polygons for point-in-polygon at [${lat}, ${lon}]`);
      ...
      if (!pointInPolyData.error && pointInPolyData.features && pointInPolyData.features.length > 0) {
        pointInPolyData.features.forEach((feature: any) => {
          ...
          if (rings && pointInPolygon(lat, lon, rings)) {
            ...
            results.push({
              ...
              facilityId: attrs.FACILITYID || attrs.facilityId || '',
              name: attrs.NAME || attrs.name || 'Unknown',
              fcode: attrs.FCODE || attrs.fcode || 'Unknown',
              ownType: attrs.OWNTYPE || attrs.ownType || 'Unknown',
              lastUpdate: attrs.LASTUPDATE || attrs.lastUpdate || '',
              subTypeField: attrs.SUBTYPEFIELD || attrs.subTypeField || 0,
              shapeArea: attrs.Shape__Area || attrs.shapeArea || 0,
              shapeLength: attrs.Shape__Length || attrs.shapeLength || 0,
        ...
        console.log(`✅ Found ${results.length} Lake County Facility Site Polygon(s) containing the point`);
      }
    } catch (error) {
    ...
    if (maxRadius > 0) {
      try {
        ...
        while (hasMore) {
          ...
          if (resultOffset === 0) {
            console.log(`🏛️ Querying Lake County Facility Site Polygons for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
          }
          
          const proximityData = await fetchJSONSmart(proximityUrl) as any;
          
          if (proximityData.error) {
            console.error('❌ Lake County Facility Site Polygons API Error:', proximityData.error);
            break;
          }
        ...
        console.log(`✅ Fetched ${allFeatures.length} total Lake County Facility Site Polygons for proximity`);
        ...
        console.log(`✅ Found ${results.filter(r => !r.containing).length} Lake County Facility Site Polygon(s) within ${maxRadius} miles`);
      } catch (error) {
    ...
    console.error('❌ Error querying Lake County Facility Site Polygons data:', error);
    return [];
  }
}
