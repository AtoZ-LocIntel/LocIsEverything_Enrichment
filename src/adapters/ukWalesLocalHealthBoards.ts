/**
 * UK Wales Local Health Boards Adapter
 * Queries UK Office for National Statistics Wales Local Health Boards polygon feature service
 * Supports point-in-polygon and proximity queries up to 25 miles
 */

import { fetchJSONSmart } from '../services/EnrichmentService';

export interface UKWalesLocalHealthBoardInfo {
  objectId: number;
  lhb23cd: string | null;
  lhb23nm: string | null;
...
export async function getUKWalesLocalHealthBoardsData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: UKWalesLocalHealthBoardInfo[];
  nearby_features: UKWalesLocalHealthBoardInfo[];
  _all: UKWalesLocalHealthBoardInfo[];
}> {
  const baseUrl = 'https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Local_Health_Boards_December_2023_WA_BFC/FeatureServer/0';
  
  const containing: UKWalesLocalHealthBoardInfo[] = [];
  const nearby_features: UKWalesLocalHealthBoardInfo[] = [];
  const _all: UKWalesLocalHealthBoardInfo[] = [];
  
  try {
    // Cap radius at 25 miles
    const effectiveRadiusMiles = Math.min(radiusMiles, 25.0);
    const radiusMeters = effectiveRadiusMiles * 1609.34;
    
    // Always run point-in-polygon query (contains query) - even if radius is 0
    const pointGeometry = {
      x: lon,
      y: lat,
    ...
    if (containsResponse && Array.isArray(containsResponse.features)) {
      for (const feature of containsResponse.features) {
        ...
        // Trust the server's esriSpatialRelIntersects query - add all returned features to containing
        const healthBoardInfo: UKWalesLocalHealthBoardInfo = {
          objectId: attributes.FID || attributes.OBJECTID || attributes.ESRI_OID || 0,
          lhb23cd: attributes.LHB23CD || attributes.lhb23cd || null,
          lhb23nm: attributes.LHB23NM || attributes.lhb23nm || null,
        ...
        containing.push(healthBoardInfo);
        _all.push(healthBoardInfo);
        seenObjectIds.add(healthBoardInfo.objectId);
      }
    }
    
    // Only run proximity search if radius > 0
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        // Skip features already added as containing
        const objectId = attributes.FID || attributes.OBJECTID || attributes.ESRI_OID || 0;
        if (seenObjectIds.has(objectId)) {
          continue;
        }
        
        // Extract Local Health Board information
        const healthBoardInfo: UKWalesLocalHealthBoardInfo = {
          objectId: objectId,
          lhb23cd: attributes.LHB23CD || attributes.lhb23cd || null,
          lhb23nm: attributes.LHB23NM || attributes.lhb23nm || null,
        ...
        if (isContaining) {
          healthBoardInfo.distance_miles = 0;
          healthBoardInfo.isContaining = true;
          containing.push(healthBoardInfo);
        } else {
          ...
          if (distance <= effectiveRadiusMiles) {
            healthBoardInfo.distance_miles = distance;
            healthBoardInfo.isContaining = false;
            nearby_features.push(healthBoardInfo);
          }
        }
