/**
 * USFS Forest Boundaries Adapter
 * Queries USFS Forest System Boundaries polygon feature service
...
export interface USFSForestBoundaryInfo {
...
/**
 * Query USFS Forest Boundaries for point-in-polygon and proximity
 * Supports proximity queries up to 50 miles
 */
export async function getUSFSForestBoundariesData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<USFSForestBoundaryInfo[]> {
  try {
    ...
    const results: USFSForestBoundaryInfo[] = [];
    ...
    try {
      ...
      console.log(`🌲 Querying USFS Forest Boundaries for point-in-polygon at [${lat}, ${lon}]`);
      console.log(`🔗 USFS Forest Boundaries Point-in-Polygon Query URL: ${pointInPolyUrl}`);
      ...
      if (!pointInPolyData.error && pointInPolyData.features && pointInPolyData.features.length > 0) {
        pointInPolyData.features.forEach((feature: any) => {
          ...
          if (isContaining) {
            ...
            const forestName = attributes.FORESTNAME || attributes.ForestName || attributes.forestname || attributes.FOREST_NM || attributes.Forest_Nm || attributes.forest_nm || null;
            const forestCode = attributes.FORESTCODE || attributes.ForestCode || attributes.forestcode || attributes.FOREST_CD || attributes.Forest_Cd || attributes.forest_cd || null;
            const regionCode = attributes.REGIONCODE || attributes.RegionCode || attributes.regioncode || attributes.REGION_CD || attributes.Region_Cd || attributes.region_cd || null;
            const regionName = attributes.REGIONNAME || attributes.RegionName || attributes.regionname || attributes.REGION_NM || attributes.Region_Nm || attributes.region_nm || null;
      ...
      console.log(`✅ Found ${results.length} USFS Forest Boundary(ies) containing the point`);
    } catch (error) {
    ...
    if (maxRadius > 0) {
      try {
        ...
        while (hasMore) {
          ...
          if (resultOffset === 0) {
            console.log(`🌲 Querying USFS Forest Boundaries for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
          }
          
          const proximityData = await fetchJSONSmart(proximityUrl) as any;
          
          if (proximityData.error) {
            console.error('❌ USFS Forest Boundaries API Error:', proximityData.error);
            break;
          }
        ...
        console.log(`✅ Fetched ${allFeatures.length} total USFS Forest Boundaries for proximity`);
    ...
    console.log(`✅ USFS Forest Boundaries: Found ${results.length} forest boundary(ies)`);
    return results;
  } catch (error) {
    console.error('❌ Error querying USFS Forest Boundaries data:', error);
    throw error;
  }
}
