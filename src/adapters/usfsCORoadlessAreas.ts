/**
 * USFS Colorado Roadless Areas Adapter
 * Queries USFS Colorado Roadless Areas polygon feature service
 * Supports point-in-polygon and proximity queries
 */

const BASE_SERVICE_URL = 'https://apps.fs.usda.gov/arcx/rest/services/EDW/EDW_ColoradoRoadlessAreas2012_01/MapServer/0';

export interface USFSCORoadlessAreaInfo {
...
/**
 * Query USFS Colorado Roadless Areas for point-in-polygon and proximity
 * Supports proximity queries up to 100 miles
 */
export async function getUSFSCORoadlessAreasData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<USFSCORoadlessAreaInfo[]> {
  try {
    const { fetchJSONSmart } = await import('../services/EnrichmentService');
    
    const maxRadius = radiusMiles ? Math.min(radiusMiles, 50.0) : 50.0;
    const results: USFSCORoadlessAreaInfo[] = [];
    ...
    try {
      ...
      console.log(`🏔️ Querying USFS Colorado Roadless Areas for point-in-polygon at [${lat}, ${lon}]`);
      ...
      if (!pointInPolyData.error && pointInPolyData.features && pointInPolyData.features.length > 0) {
        pointInPolyData.features.forEach((feature: any) => {
          ...
          if (isContaining) {
            ...
            const areaName = attributes.AREANAME || attributes.AreaName || attributes.areaname || attributes.NAME || attributes.Name || attributes.name || null;
            const areaType = attributes.AREATYPE || attributes.AreaType || attributes.areatype || attributes.TYPE || attributes.Type || attributes.type || null;
      ...
      console.log(`✅ Found ${results.length} USFS Colorado Roadless Area(s) containing the point`);
    } catch (error) {
    ...
    if (maxRadius > 0) {
      try {
        ...
        while (hasMore) {
          ...
          if (resultOffset === 0) {
            console.log(`🏔️ Querying USFS Colorado Roadless Areas for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
          }
          
          const proximityData = await fetchJSONSmart(proximityUrl) as any;
          
          if (proximityData.error) {
            console.error('❌ USFS Colorado Roadless Areas API Error:', proximityData.error);
            break;
          }
        ...
        console.log(`✅ Fetched ${allFeatures.length} total USFS Colorado Roadless Areas for proximity`);
    ...
    console.log(`✅ USFS Colorado Roadless Areas: Found ${results.length} roadless area(s)`);
    return results;
  } catch (error) {
    console.error('❌ Error querying USFS Colorado Roadless Areas data:', error);
    throw error;
  }
}
