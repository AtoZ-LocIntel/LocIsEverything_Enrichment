/**
 * NH Law Enforcement Adapter
 * Queries New Hampshire Law Enforcement facilities from NH GRANIT FeatureServer
 * Supports proximity queries to find law enforcement facilities within a specified radius
 */

const BASE_SERVICE_URL = 'https://nhgeodata.unh.edu/hosting/rest/services/Hosted/CSD_HSIP_ServiceFacilities/FeatureServer';
const LAYER_ID = 4;

export interface NHLawEnforcement {
  name: string | null;
  type: string | null;
  address: string | null;
  city: string | null;
...
/**
 * Query NH Law Enforcement FeatureServer for proximity search
 * Returns law enforcement facilities within the specified radius (in miles)
 */
export async function getNHLawEnforcementData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<NHLawEnforcement[]> {
  try {
    console.log(`🚔 Querying NH Law Enforcement facilities within ${radiusMiles} miles of [${lat}, ${lon}]`);
    ...
    console.log(`🔗 NH Law Enforcement Query URL: ${queryUrl.toString()}`);
    ...
    if (data.error) {
      console.error('❌ NH Law Enforcement API Error:', data.error);
      return [];
    }
    
    // Check if we have features
    if (!data.features || data.features.length === 0) {
      console.log(`ℹ️ No NH Law Enforcement facilities found within ${radiusMiles} miles`);
      return [];
    }
    
    // Process all features
    const lawEnforcementFacilities: NHLawEnforcement[] = data.features.map((feature: any) => {
    ...
    console.log(`✅ Found ${lawEnforcementFacilities.length} NH Law Enforcement facilities`);
    
    return lawEnforcementFacilities;
  } catch (error) {
    console.error('❌ Error querying NH Law Enforcement:', error);
    return [];
  }
}
