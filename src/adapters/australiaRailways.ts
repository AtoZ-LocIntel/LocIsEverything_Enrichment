/**
 * Australia Railways Adapter
 * Queries Digital Atlas AUS Railway Lines FeatureServer
 * Polyline feature service for railway infrastructure
 * Supports proximity queries up to 50 miles
 */

import { fetchJSONSmart } from '../services/EnrichmentService';

const BASE_SERVICE_URL = 'https://services-ap1.arcgis.com/ypkPEy1AmwPKGNNv/arcgis/rest/services/Railway_Lines_vw/FeatureServer/0';

export interface AustraliaRailwayInfo {
  objectId: number;
  name: string | null;
...
export async function getAustraliaRailwaysData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<AustraliaRailwayInfo[]> {
  try {
    console.log(`🚂 Querying Australia Railways within ${radiusMiles} miles of [${lat}, ${lon}]`);
    
    // Convert radius from miles to meters for ESRI query
    const radiusMeters = radiusMiles * 1609.34;
    
    const railways: AustraliaRailwayInfo[] = [];
    let hasMore = true;
    let resultOffset = 0;
    ...
    while (hasMore) {
      ...
      console.log(`🔗 Australia Railways Query URL: ${queryUrl.toString()}`);
      
      const response = await fetchJSONSmart(queryUrl.toString());
      
      if (response.error) {
        console.error(`❌ Australia Railways API Error:`, response.error);
        hasMore = false;
        break;
      }
      ...
      for (const feature of response.features) {
        ...
        if (finalDistance <= radiusMiles && closestPoint) {
          const railwayInfo: AustraliaRailwayInfo = {
    ...
    // Sort by distance
    railways.sort((a, b) => (a.distance_miles || Infinity) - (b.distance_miles || Infinity));
    
    console.log(`✅ Found ${railways.length} Australia Railways`);
    
    return railways;
  } catch (error) {
    console.error('❌ Error querying Australia Railways:', error);
    return [];
  }
}
