/**
 * UK Counties and Unitary Authorities Adapter
 * Queries UK Office for National Statistics Counties and Unitary Authorities polygon feature service
 * Supports point-in-polygon and proximity queries up to 50 miles
 *
 * Service: Counties_and_Unitary_Authorities_December_2021_UK_BGC_2022 (FeatureServer/0)
...
export interface UKCountyUnitaryAuthorityInfo {
  objectId: number;
  ctyua21cd: string | null;
  ctyua21nm: string | null;
  ctyua21nmw: string | null;
  bngE: number | null;
  bngN: number | null;
  long: number | null;
  lat: number | null;
...
export async function getUKCountiesUnitaryAuthoritiesData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: UKCountyUnitaryAuthorityInfo[];
  nearby_features: UKCountyUnitaryAuthorityInfo[];
  _all: UKCountyUnitaryAuthorityInfo[];
}> {
  const baseUrl = 'https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Counties_and_Unitary_Authorities_December_2021_UK_BGC_2022/FeatureServer/0';
  
  const containing: UKCountyUnitaryAuthorityInfo[] = [];
  const nearby_features: UKCountyUnitaryAuthorityInfo[] = [];
  const _all: UKCountyUnitaryAuthorityInfo[] = [];
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        const info: UKCountyUnitaryAuthorityInfo = {
          objectId: attributes.FID || attributes.OBJECTID || attributes.ESRI_OID || 0,
          ctyua21cd: attributes.CTYUA21CD || attributes.ctyua21cd || null,
          ctyua21nm: attributes.CTYUA21NM || attributes.ctyua21nm || null,
          ctyua21nmw: attributes.CTYUA21NMW || attributes.ctyua21nmw || null,
