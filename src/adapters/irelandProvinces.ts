...
export interface IrelandProvinceInfo {
  objectId: number;
  name: string;
  provinceId: number;
  guid: string;
...
export async function getIrelandProvincesData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: IrelandProvinceInfo[];
  nearby_features: IrelandProvinceInfo[];
  _all: IrelandProvinceInfo[];
}> {
  const baseUrl = 'https://services-eu1.arcgis.com/FH5XCsx8rYXqnjF5/ArcGIS/rest/services/Provinces___OSi_National_Statutory_Boundaries/FeatureServer/0';
  
  const containing: IrelandProvinceInfo[] = [];
  const nearby_features: IrelandProvinceInfo[] = [];
  const _all: IrelandProvinceInfo[] = [];
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        const provinceInfo: IrelandProvinceInfo = {
          objectId: attributes.ESRI_OID || attributes.OBJECTID || 0,
          name: attributes.PROVINCE || '',
          provinceId: attributes.PV_ID || 0,
          guid: attributes.GUID || '',
          centroidX: attributes.CENTROID_X || 0,
          centroidY: attributes.CENTROID_Y || 0,
          area: attributes.AREA || attributes.Shape__Area || 0,
          geometry: geometry,
          ...attributes
        };
        ...
        if (isContaining) {
          provinceInfo.distance_miles = 0;
          containing.push(provinceInfo);
        } else {
          ...
          if (distance <= radiusMiles) {
            provinceInfo.distance_miles = distance;
            nearby_features.push(provinceInfo);
          }
        }
        
        _all.push(provinceInfo);
      }
