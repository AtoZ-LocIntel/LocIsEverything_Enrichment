/**
 * UK Workplace Zones Adapter
 * Queries Workplace Zones December 2011 Boundaries (England & Wales) polygon feature service
 * Supports point-in-polygon and proximity queries up to 50 miles
 *
 * Service: Workplace_Zones_Dec_2011_FCB_in_England_and_Wales_2022 (FeatureServer/0)
 ...
 *
 * Based on: UK Local Authority Districts adapter
 */

import { fetchJSONSmart } from '../services/EnrichmentService';

export interface UKWorkplaceZoneInfo {
  objectId: number;
  wz11cd: string | null;
  lad11cd: string | null;
  lad11nm: string | null;
  lad11nmw: string | null;
...
function distanceToPolygon(
  ...
  for (const ring of polygon) {
    ...
    for (let i = 0; i < ring.length; i++) {
      ...
      // Coordinates are in projected meters; for distance we approximate using lat/lon input
      // Here we treat them as if they were lon/lat offsets around the query point
...
export async function getUKWorkplaceZonesData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: UKWorkplaceZoneInfo[];
  nearby_features: UKWorkplaceZoneInfo[];
  _all: UKWorkplaceZoneInfo[];
}> {
  const baseUrl = 'https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Workplace_Zones_Dec_2011_FCB_in_England_and_Wales_2022/FeatureServer/0';
  
  const containing: UKWorkplaceZoneInfo[] = [];
  const nearby_features: UKWorkplaceZoneInfo[] = [];
  const _all: UKWorkplaceZoneInfo[] = [];
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        const info: UKWorkplaceZoneInfo = {
          objectId: attributes.OBJECTID || attributes.FID || attributes.ESRI_OID || 0,
          wz11cd: attributes.wz11cd || attributes.WZ11CD || null,
          lad11cd: attributes.lad11cd || attributes.LAD11CD || null,
          lad11nm: attributes.lad11nm || attributes.LAD11NM || null,
          lad11nmw: attributes.lad11nmw || attributes.LAD11NMW || null,
