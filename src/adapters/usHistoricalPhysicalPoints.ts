/**
 * US Historical Physical Points Adapter
 * Queries GNIS Historical Physical Points FeatureServer for point features
...
const BASE_SERVICE_URL = 'https://services2.arcgis.com/FiaPA4ga0iQKduv3/arcgis/rest/services/GNIS_Historical_Physical_Points_v/FeatureServer';

export interface USHistoricalPhysicalPointInfo {
  pointId: string | null;
...
/**
 * Query US Historical Physical Points FeatureServer
 * Supports proximity queries up to 50 miles
 */
export async function getUSHistoricalPhysicalPointsData(
  lat: number,
  lon: number,
  radius?: number
): Promise<USHistoricalPhysicalPointInfo[]> {
  try {
    if (!radius || radius <= 0) {
      console.log(`ℹ️ US Historical Physical Points requires a radius for proximity query`);
      return [];
    }
    ...
    console.log(`🏔️ Querying US Historical Physical Points within ${cappedRadius} miles of [${lat}, ${lon}]`);
    
    const radiusMeters = cappedRadius * 1609.34;
    
    const results: USHistoricalPhysicalPointInfo[] = [];
    ...
    while (hasMore) {
      ...
      if (resultOffset === 0) {
        console.log(`🔗 US Historical Physical Points Proximity Query URL: ${queryUrl.toString()}`);
      }
      ...
      try {
        ...
        if (proximityData.error) {
          console.error(`❌ US Historical Physical Points API Error:`, proximityData.error);
          break;
        }
        ...
        if (resultOffset === 0) {
          console.log(`✅ US Historical Physical Points returned ${proximityData.features.length} feature(s) from proximity query`);
        }
        ...
        console.error(`❌ Error fetching US Historical Physical Points (offset ${resultOffset}):`, error);
        hasMore = false;
      }
    }
    
    console.log(`✅ Fetched ${allFeatures.length} total US Historical Physical Points features`);
    ...
    console.log(`✅ Processed ${results.length} US Historical Physical Points feature(s) within ${cappedRadius} miles`);
    
    return results;
  } catch (error) {
    console.error(`❌ Error querying US Historical Physical Points:`, error);
    return [];
  }
}
