/**
 * DE No Build Points - Bay Adapter
 * Queries Delaware No Build Points - Bay from DE FirstMap FeatureServer
...
export interface DENoBuildPointBayInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

export async function getDENoBuildPointsBayData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<DENoBuildPointBayInfo[]> {
  try {
    ...
    const features: DENoBuildPointBayInfo[] = data.features.map((feature: any) => {
    ...
    console.error('❌ Error querying DE No Build Points - Bay:', error);
    return [];
  }
}
