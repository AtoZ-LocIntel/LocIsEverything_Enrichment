/**
 * DE No Build Points - Ocean Adapter
 * Queries Delaware No Build Points - Ocean from DE FirstMap FeatureServer
...
export interface DENoBuildPointOceanInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

export async function getDENoBuildPointsOceanData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<DENoBuildPointOceanInfo[]> {
  try {
    ...
    const features: DENoBuildPointOceanInfo[] = data.features.map((feature: any) => {
    ...
    console.error('❌ Error querying DE No Build Points - Ocean:', error);
    return [];
  }
}
