/**
 * CA Brush Rabbit Range Adapter
 * Queries California Brush Rabbit Range from CDFW BIOS FeatureServer
 * Supports point-in-polygon and proximity queries
 */

const BASE_SERVICE_URL = 'https://services2.arcgis.com/Uq9r85Potqm3MfRV/arcgis/rest/services/biosds1839_fpu/FeatureServer';
const LAYER_ID = 0;

export interface CABrushRabbitRangeInfo {
  rangeId: string | null;
...
/**
 * Query CA Brush Rabbit Range FeatureServer for point-in-polygon and proximity
 */
export async function getCABrushRabbitRangeData(
  lat: number,
  lon: number,
  radius?: number
): Promise<CABrushRabbitRangeInfo[]> {
  try {
    const results: CABrushRabbitRangeInfo[] = [];
    
    // Point-in-polygon query
    console.log(`🐰 Querying CA Brush Rabbit Range for containing range at [${lat}, ${lon}]`);
    ...
    if (radius && radius > 0) {
      console.log(`🐰 Querying CA Brush Rabbit Range within ${radius} miles of [${lat}, ${lon}]`);
    ...
    console.log(`✅ Found ${results.length} CA Brush Rabbit Range/Ranges`);
    return results;
  } catch (error) {
    console.error('❌ Error querying CA Brush Rabbit Range:', error);
    return [];
  }
}
