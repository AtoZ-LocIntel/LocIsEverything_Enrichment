/**
 * DataSF — SF Parking Meters (Socrata)
 * SFMTA parking meter locations (point geometry).
 * API: https://data.sfgov.org/api/v3/views/8vzz-qzz9/query.geojson
 * Proximity via SoQL within_circle on `shape`.
 */

const RESOURCE_GEOJSON = 'https://data.sfgov.org/resource/8vzz-qzz9.geojson';
const LOCATION_FIELD = 'shape';
...
export type DatasfParkingMeter = Record<string, unknown> & {
  latitude: number;
  longitude: number;
...
export async function getDatasfParkingMetersData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<DatasfParkingMeter[]> {
  ...
  if (!response.ok) {
    throw new Error(`DataSF Parking Meters HTTP ${response.status}`);
  }
  ...
  const results: DatasfParkingMeter[] = [];
