...
const BASE_SERVICE_URL = 'https://services3.arcgis.com/HESxeTbDliKKvec2/arcgis/rest/services/LakeCounty_TaxDistricts/FeatureServer/6';

export interface LakeCountyHighSchoolDistrictInfo {
  objectId: number;
  distId: string;
  hsdid: string;
  highName: string;
...
/**
 * Query Lake County High School Districts for point-in-polygon and proximity
 * Supports proximity queries up to 25 miles
 */
export async function getLakeCountyHighSchoolDistrictsData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<LakeCountyHighSchoolDistrictInfo[]> {
  try {
    // Cap radius at 25 miles
    const maxRadius = radiusMiles ? Math.min(radiusMiles, 25.0) : 25.0;
    
    const results: LakeCountyHighSchoolDistrictInfo[] = [];
    ...
    try {
      ...
      console.log(`🏫 Querying Lake County High School Districts for point-in-polygon at [${lat}, ${lon}]`);
      ...
      if (!pointInPolyData.error && pointInPolyData.features && pointInPolyData.features.length > 0) {
        pointInPolyData.features.forEach((feature: any) => {
          ...
          if (rings && pointInPolygon(lat, lon, rings)) {
            ...
            results.push({
              ...
              distId: attrs.DIST_ID || attrs.distId || '',
              hsdid: attrs.HSDID || attrs.hsdid || '',
              highName: attrs.HIGH_NAME || attrs.highName || 'Unknown',
              highDist: attrs.HIGH_DIST || attrs.highDist || '',
              district: attrs.DISTRICT || attrs.district || '',
              name: attrs.NAME || attrs.name || 'Unknown',
              addr: attrs.ADDR || attrs.addr || '',
              addr2: attrs.ADDR2 || attrs.addr2 || '',
              city: attrs.CITY || attrs.city || '',
              zip: attrs.ZIP || attrs.zip || '',
              phone: attrs.PHONE || attrs.phone || '',
              fax: attrs.FAX || attrs.fax || '',
              email: attrs.EMAIL || attrs.email || '',
              url: attrs.URL || attrs.url || '',
              mapUrl: attrs.MAPURL || attrs.mapUrl || '',
        ...
        console.log(`✅ Found ${results.length} Lake County High School District(s) containing the point`);
      }
    } catch (error) {
    ...
    if (maxRadius > 0) {
      try {
        ...
        while (hasMore) {
          ...
          if (resultOffset === 0) {
            console.log(`🏫 Querying Lake County High School Districts for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
          }
          
          const proximityData = await fetchJSONSmart(proximityUrl) as any;
          
          if (proximityData.error) {
            console.error('❌ Lake County High School Districts API Error:', proximityData.error);
            break;
          }
        ...
        console.log(`✅ Fetched ${allFeatures.length} total Lake County High School Districts for proximity`);
        ...
        console.log(`✅ Found ${results.filter(r => !r.containing).length} Lake County High School District(s) within ${maxRadius} miles`);
      } catch (error) {
    ...
    console.error('❌ Error querying Lake County High School Districts data:', error);
    return [];
  }
}
