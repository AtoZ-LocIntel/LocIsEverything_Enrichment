/**
 * CA Fire Perimeters (All) Adapter
 * Queries California Fire Perimeters (all) from CA Open Data Portal FeatureServer
...
/**
 * Query CA Fire Perimeters (All) FeatureServer for point-in-polygon and proximity
 */
export async function getCAFirePerimetersAllData(
  lat: number,
  lon: number,
  radius?: number
  ...
  try {
    ...
    console.log(`🔥 Querying CA Fire Perimeters (All) for containing fire at [${lat}, ${lon}]`);
    ...
    if (radius && radius > 0) {
      console.log(`🔥 Querying CA Fire Perimeters (All) within ${radius} miles of [${lat}, ${lon}]`);
    ...
    console.log(`✅ Found ${results.length} CA Fire Perimeter(s) (All)`);
    return results;
  } catch (error) {
    console.error('❌ Error querying CA Fire Perimeters (All):', error);
    return [];
  }
}
