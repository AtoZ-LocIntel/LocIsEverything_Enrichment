/**
 * DE No Build Line - Ocean Adapter
 * Queries Delaware No Build Line - Ocean from DE FirstMap FeatureServer
...
export interface DENoBuildLineOceanInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

export async function getDENoBuildLineOceanData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<DENoBuildLineOceanInfo[]> {
  try {
    ...
    const features: DENoBuildLineOceanInfo[] = data.features.map((feature: any) => {
    ...
    console.error('❌ Error querying DE No Build Line - Ocean:', error);
    return [];
  }
}
