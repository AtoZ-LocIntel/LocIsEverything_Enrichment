/**
 * TIGER CBSA Adapter
 * Queries US Census TIGER Core Based Statistical Areas MapServer
...
const BASE_SERVICE_URL = 'https://tigerweb.geo.census.gov/arcgis/rest/services/TIGERweb/CBSA/MapServer';

export interface TIGERCBSAInfo {
  objectId: number;
  name?: string | null;
...
/**
 * Query TIGER CBSA data for a given location and radius
 */
export async function getTIGERCBSAData(
  layerId: number,
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing?: TIGERCBSAInfo | null;
  ...
  try {
    ...
    // ESRI expects geometry in WGS84 Web Mercator (3857) or WGS84 (4326)
    // We'll use a simple point buffer approach
    const geometry = {
      x: lon,
      y: lat,
    ...
    while (true) {
      ...
      const features = response.features.map((feature: any) => {
        ...
        const name = attrs.NAME || attrs.CBSA || attrs.NAME20 || attrs.NAMELSAD || attrs.CSANAME || attrs.NECTANAME || null;
        
        // Extract state/county FIPS codes
        const stateFips = attrs.STATEFP || attrs.STATE || null;
        const countyFips = attrs.COUNTYFP || attrs.COUNTY || null;
        
        // Extract CBSA type
        const cbsaType = attrs.TYPE || attrs.CBSATYPE || attrs.LSAD || null;
    ...
    let containingFeature: TIGERCBSAInfo | null = null;
    ...
    return {
      containing: containingFeature,
      nearby_features,
      _all
    };
  } catch (error) {
    console.error(`Error fetching TIGER CBSA data for layer ${layerId}:`, error);
    return {
      containing: null,
      nearby_features: [],
      _all: []
