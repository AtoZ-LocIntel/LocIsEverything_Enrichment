/**
 * South Carolina Lakes and Reservoirs Adapter
 * Queries SC Lakes and Reservoirs polygons from ArcGIS FeatureServer
 * Supports point-in-polygon and proximity queries up to 50 miles
 * Layer: Lakes_and_Reservoirs (Layer 0) - Polygons
 */

const BASE_SERVICE_URL = 'https://services.arcgis.com/acgZYxoN5Oj8pDLa/ArcGIS/rest/services/Lakes_and_Reservoirs/FeatureServer';
const LAYER_ID = 0;

export interface SCLakesReservoirsInfo {
...
/**
 * Query SC Lakes and Reservoirs for point-in-polygon and proximity
 * Supports proximity queries up to 50 miles
 */
export async function getSCLakesReservoirsData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<SCLakesReservoirsInfo[]> {
  try {
    ...
    const results: SCLakesReservoirsInfo[] = [];
    ...
    try {
      ...
      console.log(`🏞️ Querying SC Lakes and Reservoirs for point-in-polygon at [${lat}, ${lon}]`);
      
      const pipData = await fetchJSONSmart(pipUrl) as any;
      
      if (pipData.error) {
        console.error('❌ SC Lakes and Reservoirs API Error:', pipData.error);
        ...
        console.log(`✅ Found ${results.length} SC Lake/Reservoir(s) containing the point`);
      }
    } catch (error) {
      console.error('❌ Point-in-polygon query failed for SC Lakes and Reservoirs:', error);
    }
    ...
    try {
      ...
      while (hasMore) {
        ...
        if (resultOffset === 0) {
          console.log(`🏞️ Querying SC Lakes and Reservoirs for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
        }
        
        const proximityData = await fetchJSONSmart(proximityUrl) as any;
        
        if (proximityData.error) {
          console.error('❌ SC Lakes and Reservoirs API Error:', proximityData.error);
          break;
        }
      ...
      console.log(`✅ Fetched ${allFeatures.length} total SC Lakes and Reservoirs features for proximity`);
      ...
      console.log(`✅ Found ${results.length} SC Lake/Reservoir(s) within ${maxRadius} miles`);
    } catch (error) {
      console.error('❌ Proximity query failed for SC Lakes and Reservoirs:', error);
    }
    
    return results;
  } catch (error) {
    console.error('❌ Error querying SC Lakes and Reservoirs data:', error);
    return [];
  }
}
