/**
 * NH Emergency Medical Services (EMS) Adapter
 * Queries New Hampshire EMS facilities from NH GRANIT FeatureServer
 * Supports proximity queries to find EMS facilities within a specified radius
 */

const BASE_SERVICE_URL = 'https://nhgeodata.unh.edu/hosting/rest/services/Hosted/CSD_HSIP_ServiceFacilities/FeatureServer';
const LAYER_ID = 1;

export interface NHEMSFacility {
  name: string | null;
  type: string | null;
  address: string | null;
  city: string | null;
...
/**
 * Query NH EMS FeatureServer for proximity search
 * Returns EMS facilities within the specified radius (in miles)
 */
export async function getNHEMSData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<NHEMSFacility[]> {
  try {
    console.log(`🚑 Querying NH EMS facilities within ${radiusMiles} miles of [${lat}, ${lon}]`);
    ...
    console.log(`🔗 NH EMS Query URL: ${queryUrl.toString()}`);
    ...
    if (data.error) {
      console.error('❌ NH EMS API Error:', data.error);
      return [];
    }
    
    // Check if we have features
    if (!data.features || data.features.length === 0) {
      console.log(`ℹ️ No NH EMS facilities found within ${radiusMiles} miles`);
      return [];
    }
    
    // Process all features
    const emsFacilities: NHEMSFacility[] = data.features.map((feature: any) => {
    ...
    console.log(`✅ Found ${emsFacilities.length} NH EMS facilities`);
    
    return emsFacilities;
  } catch (error) {
    console.error('❌ Error querying NH EMS:', error);
    return [];
  }
}
