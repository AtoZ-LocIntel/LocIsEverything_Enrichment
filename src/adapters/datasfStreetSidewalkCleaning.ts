/**
 * DataSF — Street and Sidewalk Cleaning (Socrata)
 * Service requests with geocoded `point` locations.
 * API: https://data.sfgov.org/api/v3/views/h3eg-w3pj/query.geojson
 * Proximity via SoQL within_circle on `point`.
 */

const RESOURCE_GEOJSON = 'https://data.sfgov.org/resource/h3eg-w3pj.geojson';
...
export type DatasfStreetSidewalkCleaning = Record<string, unknown> & {
  latitude: number;
  longitude: number;
...
export async function getDatasfStreetSidewalkCleaningData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<DatasfStreetSidewalkCleaning[]> {
  ...
  if (!response.ok) {
    throw new Error(`DataSF Street and Sidewalk Cleaning HTTP ${response.status}`);
  }
  ...
  const results: DatasfStreetSidewalkCleaning[] = [];
  ...
  for (const feature of data.features) {
    ...
    const row: DatasfStreetSidewalkCleaning = {
      ...props,
      latitude: plat,
      longitude: plon,
