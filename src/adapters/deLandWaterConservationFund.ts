/**
 * DE Land and Water Conservation Fund Adapter
...
export interface DELandWaterConservationFundData {
  containingFeature: DELandWaterConservationFundInfo | null;
  nearbyFeatures: DELandWaterConservationFundInfo[];
}

export interface DELandWaterConservationFundInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

async function getContainingFeature(lat: number, lon: number): Promise<DELandWaterConservationFundInfo | null> {
  try {
    ...
    console.error('❌ Error querying DE Land and Water Conservation Fund:', error);
    return null;
  }
}

async function getNearbyFeatures(lat: number, lon: number, radiusMiles: number): Promise<DELandWaterConservationFundInfo[]> {
  try {
    ...
    const features: DELandWaterConservationFundInfo[] = data.features.map((feature: any) => {
...
export async function getDELandWaterConservationFundData(
  lat: number,
  lon: number,
  radiusMiles: number = 0
): Promise<DELandWaterConservationFundData> {
