/**
 * USFS Recreation Area Activities Adapter
 * Queries USFS Recreation Area Activities point feature service
 * Supports proximity queries up to 100 miles
 */

const BASE_SERVICE_URL = 'https://apps.fs.usda.gov/arcx/rest/services/EDW/EDW_RecreationAreaActivities_01/MapServer/0';

export interface USFSRecreationAreaActivityInfo {
...
/**
 * Query USFS Recreation Area Activities within proximity of a location
 * Supports proximity queries up to 100 miles
 */
export async function getUSFSRecreationAreaActivitiesData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<USFSRecreationAreaActivityInfo[]> {
  try {
    ...
    while (hasMore) {
      ...
      if (resultOffset === 0) {
        console.log(`🎯 Querying USFS Recreation Area Activities for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
      }
      
      const proximityData = await fetchJSONSmart(proximityUrl) as any;
      
      if (proximityData.error) {
        console.error('❌ USFS Recreation Area Activities API Error:', proximityData.error);
        break;
      }
    ...
    console.log(`✅ Fetched ${allFeatures.length} total USFS Recreation Area Activities for proximity`);
    
    const results: USFSRecreationAreaActivityInfo[] = [];
    ...
    allFeatures.forEach((feature: any) => {
      ...
      if (geometry && geometry.x !== undefined && geometry.y !== undefined) {
        ...
        if (distance <= maxRadius) {
          ...
          const activityName = attributes.ACTIVITYNAME || attributes.ActivityName || attributes.activityname || null;
          const parentActivityName = attributes.PARENTACTIVITYNAME || attributes.ParentActivityName || attributes.parentactivityname || null;
    ...
    console.log(`✅ USFS Recreation Area Activities: Found ${results.length} activity(ies)`);
    return results;
  } catch (error) {
    console.error('❌ Error querying USFS Recreation Area Activities data:', error);
    throw error;
  }
}
