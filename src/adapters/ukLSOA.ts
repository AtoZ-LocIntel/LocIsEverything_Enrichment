...
export async function getUKLSOAData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: UKLSOAInfo[];
  nearby_features: UKLSOAInfo[];
  _all: UKLSOAInfo[];
}> {
  const baseUrl = 'https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/LSOA_2021_EW_BSC_V4_RUC/FeatureServer/0';
  
  const containing: UKLSOAInfo[] = [];
  const nearby_features: UKLSOAInfo[] = [];
  const _all: UKLSOAInfo[] = [];
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        const info: UKLSOAInfo = {
          objectId: attributes.FID || attributes.OBJECTID || attributes.ESRI_OID || 0,
          lsoa21cd: attributes.LSOA21CD || attributes.lsoa21cd || null,
          lsoa21nm: attributes.LSOA21NM || attributes.lsoa21nm || null,
          lsoa21nmw: attributes.LSOA21NMW || attributes.lsoa21nmw || null,
          ...
          ruc21cd: attributes.RUC21CD || attributes.ruc21cd || null,
          ruc21nm: attributes.RUC21NM || attributes.ruc21nm || null,
          urban_rural: attributes.Urban_rura || attributes.urban_rura || null,
