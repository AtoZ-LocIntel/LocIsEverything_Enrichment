/**
 * DE Permanent Restricted Areas Adapter
...
export interface DEPermanentRestrictedAreaData {
  containingFeature: DEPermanentRestrictedAreaInfo | null;
  nearbyFeatures: DEPermanentRestrictedAreaInfo[];
}

export interface DEPermanentRestrictedAreaInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

async function getContainingFeature(lat: number, lon: number): Promise<DEPermanentRestrictedAreaInfo | null> {
  try {
    ...
    console.error('❌ Error querying DE Permanent Restricted Areas:', error);
    return null;
  }
}

async function getNearbyFeatures(lat: number, lon: number, radiusMiles: number): Promise<DEPermanentRestrictedAreaInfo[]> {
  try {
    ...
    const features: DEPermanentRestrictedAreaInfo[] = data.features.map((feature: any) => {
...
export async function getDEPermanentRestrictedAreasData(
  lat: number,
  lon: number,
  radiusMiles: number = 0
): Promise<DEPermanentRestrictedAreaData> {
