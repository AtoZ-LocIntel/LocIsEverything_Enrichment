...
const BASE_SERVICE_URL = 'https://services3.arcgis.com/HESxeTbDliKKvec2/arcgis/rest/services/LakeCounty_Planimetrics/FeatureServer/0';

export interface LakeCountyPavementBoundaryInfo {
  objectId: number;
  type: string;
  shapeArea: number;
  shapeLength: number;
...
/**
 * Query Lake County Pavement Boundaries for point-in-polygon and proximity
 * Supports proximity queries up to 1 mile
 */
export async function getLakeCountyPavementBoundariesData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<LakeCountyPavementBoundaryInfo[]> {
  try {
    // Cap radius at 1 mile
    const maxRadius = radiusMiles ? Math.min(radiusMiles, 1.0) : 1.0;
    
    const results: LakeCountyPavementBoundaryInfo[] = [];
    ...
    try {
      ...
      console.log(`🛣️ Querying Lake County Pavement Boundaries for point-in-polygon at [${lat}, ${lon}]`);
      ...
      if (!pointInPolyData.error && pointInPolyData.features && pointInPolyData.features.length > 0) {
        pointInPolyData.features.forEach((feature: any) => {
          ...
          if (rings && pointInPolygon(lat, lon, rings)) {
            ...
            results.push({
              ...
              type: feature.attributes.TYPE || feature.attributes.type || 'Unknown',
        ...
        console.log(`✅ Found ${results.length} Lake County Pavement Boundary(ies) containing the point`);
      }
    } catch (error) {
    ...
    if (maxRadius > 0) {
      try {
        ...
        while (hasMore) {
          ...
          if (resultOffset === 0) {
            console.log(`🛣️ Querying Lake County Pavement Boundaries for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
          }
          
          const proximityData = await fetchJSONSmart(proximityUrl) as any;
          
          if (proximityData.error) {
            console.error('❌ Lake County Pavement Boundaries API Error:', proximityData.error);
            break;
          }
        ...
        console.log(`✅ Fetched ${allFeatures.length} total Lake County Pavement Boundaries for proximity`);
        ...
        console.log(`✅ Found ${results.filter(r => !r.containing).length} Lake County Pavement Boundary(ies) within ${maxRadius} miles`);
      } catch (error) {
    ...
    console.error('❌ Error querying Lake County Pavement Boundaries data:', error);
    return [];
  }
}
