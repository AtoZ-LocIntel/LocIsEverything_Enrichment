/**
 * BLM National GTLF Public Managed Trails Adapter
 * Queries BLM National GTLF Public Managed Trails linear feature service
 * Supports proximity queries up to 25 miles
 */

const BASE_SERVICE_URL = 'https://services1.arcgis.com/KbxwQRRfWyEYLgp4/arcgis/rest/services/BLM_Natl_GTLF_Public_Managed_Trails/FeatureServer/2';

export interface BLMNationalTrailInfo {
...
/**
 * Query BLM National GTLF Public Managed Trails within proximity of a location
 * Supports proximity queries up to 25 miles
 */
export async function getBLMNationalTrailsData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<BLMNationalTrailInfo[]> {
  try {
    ...
    const features: BLMNationalTrailInfo[] = [];
    ...
    try {
      ...
      while (hasMore) {
        ...
        if (resultOffset === 0) {
          console.log(`🥾 Querying BLM National GTLF Public Managed Trails for proximity (${radiusMiles} miles) at [${lat}, ${lon}]`);
        }
        console.log(`🔗 BLM National Trails Proximity Query URL (offset ${resultOffset}): ${proximityUrl}`);
        
        const proximityData = await fetchJSONSmart(proximityUrl) as any;
        
        // Log the full response for debugging
        console.log(`📊 BLM National Trails Proximity Response (offset ${resultOffset}):`, {
        ...
        if (proximityData.error) {
          console.error('❌ BLM National Trails API Error:', proximityData.error);
          break;
        }
        ...
        console.log(`📦 Fetched batch: ${proximityData.features.length} trail segments (total so far: ${allFeatures.length})`);
      ...
      console.log(`✅ Fetched ${allFeatures.length} total BLM National Trail segments (${Math.ceil(allFeatures.length / batchSize)} batches)`);
      ...
      console.warn('⚠️ BLM National Trails: Proximity query failed:', error);
    }
    
    // Sort by distance
    features.sort((a, b) => (a.distance_miles || 0) - (b.distance_miles || 0));
    
    console.log(`✅ BLM National Trails: Found ${features.length} trail segment(s)`);
    return features;
  } catch (error) {
    console.error('❌ Error querying BLM National Trails data:', error);
    throw error;
  }
}
