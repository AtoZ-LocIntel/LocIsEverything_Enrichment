/**
 * DE Seasonal Restricted Areas Adapter
...
export interface DESeasonalRestrictedAreaData {
  containingFeature: DESeasonalRestrictedAreaInfo | null;
  nearbyFeatures: DESeasonalRestrictedAreaInfo[];
}

export interface DESeasonalRestrictedAreaInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

async function getContainingFeature(lat: number, lon: number): Promise<DESeasonalRestrictedAreaInfo | null> {
  try {
    ...
    console.error('❌ Error querying DE Seasonal Restricted Areas:', error);
    return null;
  }
}

async function getNearbyFeatures(lat: number, lon: number, radiusMiles: number): Promise<DESeasonalRestrictedAreaInfo[]> {
  try {
    ...
    const features: DESeasonalRestrictedAreaInfo[] = data.features.map((feature: any) => {
...
export async function getDESeasonalRestrictedAreasData(
  lat: number,
  lon: number,
  radiusMiles: number = 0
): Promise<DESeasonalRestrictedAreaData> {
