/**
 * CA Condor Range Adapter
 * Queries California Condor Range from CDFW BIOS FeatureServer
 * Supports point-in-polygon and proximity queries
 */

const BASE_SERVICE_URL = 'https://services2.arcgis.com/Uq9r85Potqm3MfRV/arcgis/rest/services/biosds916_fpu/FeatureServer';
const LAYER_ID = 0;

export interface CACondorRangeInfo {
  rangeId: string | null;
...
/**
 * Query CA Condor Range FeatureServer for point-in-polygon and proximity
 */
export async function getCACondorRangeData(
  lat: number,
  lon: number,
  radius?: number
): Promise<CACondorRangeInfo[]> {
  try {
    const results: CACondorRangeInfo[] = [];
    
    // Point-in-polygon query
    console.log(`🦅 Querying CA Condor Range for containing range at [${lat}, ${lon}]`);
    ...
    if (radius && radius > 0) {
      console.log(`🦅 Querying CA Condor Range within ${radius} miles of [${lat}, ${lon}]`);
    ...
    console.log(`✅ Found ${results.length} CA Condor Range/Ranges`);
    return results;
  } catch (error) {
    console.error('❌ Error querying CA Condor Range:', error);
    return [];
  }
}
