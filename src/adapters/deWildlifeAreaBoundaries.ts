/**
 * DE Wildlife Area Boundaries Adapter
...
export interface DEWildlifeAreaBoundaryData {
  containingFeature: DEWildlifeAreaBoundaryInfo | null;
  nearbyFeatures: DEWildlifeAreaBoundaryInfo[];
}

export interface DEWildlifeAreaBoundaryInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

async function getContainingFeature(lat: number, lon: number): Promise<DEWildlifeAreaBoundaryInfo | null> {
  try {
    ...
    console.error('❌ Error querying DE Wildlife Area Boundaries:', error);
    return null;
  }
}

async function getNearbyFeatures(lat: number, lon: number, radiusMiles: number): Promise<DEWildlifeAreaBoundaryInfo[]> {
  try {
    ...
    const features: DEWildlifeAreaBoundaryInfo[] = data.features.map((feature: any) => {
...
export async function getDEWildlifeAreaBoundariesData(
  lat: number,
  lon: number,
  radiusMiles: number = 0
): Promise<DEWildlifeAreaBoundaryData> {
