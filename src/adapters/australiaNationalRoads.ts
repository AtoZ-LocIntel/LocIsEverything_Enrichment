/**
 * Australia National Roads Adapter
 * Queries Digital Atlas AUS National Roads FeatureServer
 * Polyline feature service for road infrastructure
 * Supports proximity queries up to 1 mile (0.25, 0.50, 0.75, 1.0)
 */

import { fetchJSONSmart } from '../services/EnrichmentService';

const BASE_SERVICE_URL = 'https://services-ap1.arcgis.com/ypkPEy1AmwPKGNNv/arcgis/rest/services/National_Roads/FeatureServer/0';

export interface AustraliaNationalRoadInfo {
  objectId: number;
  roadId: string | null;
...
export async function getAustraliaNationalRoadsData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<AustraliaNationalRoadInfo[]> {
  try {
    console.log(`🛣️ Querying Australia National Roads within ${radiusMiles} miles of [${lat}, ${lon}]`);
    
    // Convert radius from miles to meters for ESRI query
    const radiusMeters = radiusMiles * 1609.34;
    
    const roads: AustraliaNationalRoadInfo[] = [];
    let hasMore = true;
    let resultOffset = 0;
    ...
    while (hasMore) {
      ...
      console.log(`🔗 Australia National Roads Query URL: ${queryUrl.toString()}`);
      
      const response = await fetchJSONSmart(queryUrl.toString());
      
      if (response.error) {
        console.error(`❌ Australia National Roads API Error:`, response.error);
        hasMore = false;
        break;
      }
      ...
      for (const feature of response.features) {
        ...
        if (finalDistance <= radiusMiles && closestPoint) {
          const roadInfo: AustraliaNationalRoadInfo = {
    ...
    console.log(`✅ Found ${roads.length} Australia National Roads`);
    
    return roads;
  } catch (error) {
    console.error('❌ Error querying Australia National Roads:', error);
    return [];
  }
}
