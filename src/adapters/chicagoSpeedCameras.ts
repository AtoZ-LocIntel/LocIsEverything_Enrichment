/**
 * Adapter for Chicago Speed Camera Locations data from Socrata API
 * Dataset: Speed Camera Locations
 * API endpoint: https://data.cityofchicago.org/api/v3/views/4i42-qv3h/query.json
 * SODA2 format: https://data.cityofchicago.org/resource/4i42-qv3h.json
 */

import { fetchJSONSmart } from '../services/EnrichmentService';

// Socrata API endpoint - using SODA2 format
const BASE_API_URL = 'https://data.cityofchicago.org/resource/4i42-qv3h';

export interface ChicagoSpeedCameraFeature {
  camera_id?: string;
  address?: string;
  location?: {
    type: string;
...
export type ChicagoSpeedCamerasResponse = ChicagoSpeedCameraFeature[];
...
/**
 * Query Chicago Speed Camera Locations within proximity of a location
 * Uses Socrata API with bounding box and distance filtering
 */
export async function getChicagoSpeedCamerasData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<ChicagoSpeedCameraFeature[]> {
  try {
    ...
    console.log(`🔍 Chicago Speed Cameras: Querying with spatial filter (within_circle): ${queryUrl}`);
    
    let response: ChicagoSpeedCamerasResponse;
    try {
      response = await fetchJSONSmart(queryUrl) as ChicagoSpeedCamerasResponse;
    } catch (error: any) {
      // If within_circle fails with 403 or other error, fall back to bounding box approach
      if (error?.message?.includes('403') || error?.message?.includes('Forbidden')) {
        console.warn('⚠️ Chicago Speed Cameras: within_circle query returned 403, trying bounding box approach');
      } else {
        throw error;
      }
      ...
      console.log(`🔍 Chicago Speed Cameras: Trying bounding box query: ${bboxQueryUrl}`);
      response = await fetchJSONSmart(bboxQueryUrl) as ChicagoSpeedCamerasResponse;
    }

    if (!response || !Array.isArray(response)) {
      console.warn('⚠️ Chicago Speed Cameras: No data in response');
      return [];
    }
    ...
    console.log(`✅ Chicago Speed Cameras: Filtered to ${filteredFeatures.length} features within ${radiusMiles} miles`);
    return filteredFeatures;
  } catch (error) {
    console.error('❌ Error querying Chicago Speed Cameras data:', error);
    throw error;
  }
}
