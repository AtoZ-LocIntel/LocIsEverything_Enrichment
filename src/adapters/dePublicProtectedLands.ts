/**
 * DE Public Protected Lands Adapter
...
export interface DEPublicProtectedLandData {
  containingFeature: DEPublicProtectedLandInfo | null;
  nearbyFeatures: DEPublicProtectedLandInfo[];
}

export interface DEPublicProtectedLandInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

async function getContainingFeature(lat: number, lon: number): Promise<DEPublicProtectedLandInfo | null> {
  try {
    ...
    console.error('❌ Error querying DE Public Protected Lands:', error);
    return null;
  }
}

async function getNearbyFeatures(lat: number, lon: number, radiusMiles: number): Promise<DEPublicProtectedLandInfo[]> {
  try {
    ...
    const features: DEPublicProtectedLandInfo[] = data.features.map((feature: any) => {
...
export async function getDEPublicProtectedLandsData(
  lat: number,
  lon: number,
  radiusMiles: number = 0
): Promise<DEPublicProtectedLandData> {
