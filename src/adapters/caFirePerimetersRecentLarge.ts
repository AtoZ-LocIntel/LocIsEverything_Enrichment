/**
 * CA Recent Large Fire Perimeters (GT 5000 acres) Adapter
 * Queries California Recent Large Fire Perimeters from CA Open Data Portal FeatureServer
...
/**
 * Query CA Recent Large Fire Perimeters FeatureServer for point-in-polygon and proximity
 */
export async function getCAFirePerimetersRecentLargeData(
  lat: number,
  lon: number,
  radius?: number
  ...
  try {
    ...
    console.log(`🔥 Querying CA Recent Large Fire Perimeters for containing fire at [${lat}, ${lon}]`);
    ...
    if (radius && radius > 0) {
      console.log(`🔥 Querying CA Recent Large Fire Perimeters within ${radius} miles of [${lat}, ${lon}]`);
    ...
    console.log(`✅ Found ${results.length} CA Recent Large Fire Perimeter(s)`);
    return results;
  } catch (error) {
    console.error('❌ Error querying CA Recent Large Fire Perimeters:', error);
    return [];
  }
}
