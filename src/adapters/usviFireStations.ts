/**
 * USVI Fire Stations Adapter
 * Queries USVI Fire Stations from ArcGIS FeatureServer
 * Supports proximity queries to find fire stations within a specified radius
 */

const BASE_SERVICE_URL = 'https://services.arcgis.com/XG15cJAlne2vxtgt/ArcGIS/rest/services/Fire_Stations_USVI/FeatureServer';
const LAYER_ID = 2;

export interface USVIFireStation {
...
/**
 * Query USVI Fire Stations FeatureServer for proximity search
 * Returns fire stations within the specified radius (in miles)
 */
export async function getUSVIFireStationsData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<USVIFireStation[]> {
  try {
    console.log(`🚒 Querying USVI Fire Stations within ${radiusMiles} miles of [${lat}, ${lon}]`);
    ...
    console.log(`🔗 USVI Fire Stations Query URL: ${queryUrl.toString()}`);
    ...
    if (data.error) {
      console.error('❌ USVI Fire Stations API Error:', data.error);
      return [];
    }
    
    // Check if we have features
    if (!data.features || data.features.length === 0) {
      console.log(`ℹ️ No USVI Fire Stations found within ${radiusMiles} miles`);
      return [];
    }
    
    // Process all features
    const fireStations: USVIFireStation[] = data.features.map((feature: any) => {
    ...
    console.log(`✅ Found ${fireStations.length} USVI Fire Stations`);
    
    return fireStations;
  } catch (error) {
    console.error('❌ Error querying USVI Fire Stations:', error);
    return [];
  }
}
