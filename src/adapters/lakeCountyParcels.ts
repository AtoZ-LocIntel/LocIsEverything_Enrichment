...
const BASE_SERVICE_URL = 'https://services3.arcgis.com/HESxeTbDliKKvec2/arcgis/rest/services/OpenData_ParcelPolygons/FeatureServer/0';

export interface LakeCountyParcelInfo {
  objectId: number;
...
/**
 * Query Lake County Parcels for point-in-polygon and proximity
 * Supports proximity queries up to 1 mile
 */
export async function getLakeCountyParcelsData(
  lat: number,
  lon: number,
  radiusMiles?: number
): Promise<LakeCountyParcelInfo[]> {
  try {
    // Cap radius at 1 mile
    const maxRadius = radiusMiles ? Math.min(radiusMiles, 1.0) : 1.0;
    
    const results: LakeCountyParcelInfo[] = [];
    ...
    try {
      ...
      console.log(`🏘️ Querying Lake County Parcels for point-in-polygon at [${lat}, ${lon}]`);
      ...
      if (!pointInPolyData.error && pointInPolyData.features && pointInPolyData.features.length > 0) {
        ...
        console.log(`✅ Found ${results.length} Lake County Parcel(s) containing the point`);
      }
    } catch (error) {
    ...
    if (maxRadius > 0) {
      try {
        ...
        while (hasMore) {
          ...
          if (resultOffset === 0) {
            console.log(`🏘️ Querying Lake County Parcels for proximity (${maxRadius} miles) at [${lat}, ${lon}]`);
          }
          
          const proximityData = await fetchJSONSmart(proximityUrl) as any;
          
          if (proximityData.error) {
            console.error('❌ Lake County Parcels API Error:', proximityData.error);
            break;
          }
        ...
        console.log(`✅ Fetched ${allFeatures.length} total Lake County Parcels for proximity`);
        ...
        console.log(`✅ Found ${results.filter(r => !r.containing).length} Lake County Parcel(s) within ${maxRadius} miles`);
      } catch (error) {
    ...
    console.error('❌ Error querying Lake County Parcels data:', error);
    return [];
  }
}
