/**
 * DE Natural Areas Adapter
 * Queries Delaware Natural Areas from DE FirstMap FeatureServer
...
export interface DENaturalAreaData {
  containingFeature: DENaturalAreaInfo | null;
  nearbyFeatures: DENaturalAreaInfo[];
}

export interface DENaturalAreaInfo {
...
async function getContainingFeature(
  lat: number,
  lon: number
): Promise<DENaturalAreaInfo | null> {
  try {
    ...
    console.error('❌ Error querying DE Natural Areas:', error);
    return null;
  }
}
...
async function getNearbyFeatures(
  ...
  try {
    ...
    const features: DENaturalAreaInfo[] = data.features.map((feature: any) => {
...
export async function getDENaturalAreasData(
  lat: number,
  lon: number,
  radiusMiles: number = 0
): Promise<DENaturalAreaData> {
