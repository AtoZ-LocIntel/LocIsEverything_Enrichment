/**
 * MA DEP Wetlands Adapter
 * Queries Massachusetts Department of Environmental Protection (DEP) Wetland Areas
...
const BASE_SERVICE_URL = 'https://arcgisserver.digital.mass.gov/arcgisserver/rest/services/AGOL/DEP_Wetlands/FeatureServer';
const LAYER_ID = 1;

export interface MADEPWetland {
  objectId?: number;
  globalId?: string;
  wetCode?: number;
  itValc?: string;
...
/**
 * Query MA DEP Wetlands FeatureServer for point-in-polygon (containing)
 */
export async function getMADEPWetlandsContainingData(
  lat: number,
  lon: number
): Promise<MADEPWetland[]> {
  try {
    console.log(`🌊 Querying MA DEP Wetlands containing [${lat}, ${lon}]`);
    ...
    console.log(`🔗 MA DEP Wetlands Query URL: ${queryUrl.toString()}`);
    ...
    if (data.error) {
      console.error('❌ MA DEP Wetlands API Error:', data.error);
      return [];
    }
    
    if (!data.features || data.features.length === 0) {
      console.log(`ℹ️ No MA DEP Wetlands found containing the point`);
      return [];
    }
    
    // Process features
    const wetlands: MADEPWetland[] = data.features.map((feature: any) => {
      const attributes = feature.attributes || {};
      
      return {
        objectId: attributes.OBJECTID,
        globalId: attributes.GLOBALID,
        wetCode: attributes.WETCODE,
        itValc: attributes.IT_VALC,
        itValDesc: attributes.IT_VALDESC,
        polyCode: attributes.POLY_CODE,
        source: attributes.SOURCE,
        sourceScale: attributes.SOURCE_SCALE,
        areaAcres: attributes.AREAACRES,
        areaSqMi: attributes.AREASQMI,
    ...
    console.log(`✅ Found ${wetlands.length} MA DEP Wetlands containing the point`);
    
    return wetlands;
  } catch (error) {
    console.error('❌ Error querying MA DEP Wetlands (containing):', error);
    return [];
  }
}

/**
 * Query MA DEP Wetlands FeatureServer for proximity search
 * Returns wetlands within the specified radius (in miles)
 */
export async function getMADEPWetlandsNearbyData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<MADEPWetland[]> {
  try {
    console.log(`🌊 Querying MA DEP Wetlands within ${radiusMiles} miles of [${lat}, ${lon}]`);
    ...
    if (!data.features || data.features.length === 0) {
      console.log(`ℹ️ No MA DEP Wetlands found within ${radiusMiles} miles`);
      return [];
    }
    ...
    console.log(`✅ Found ${wetlands.length} MA DEP Wetlands`);
    
    return wetlands;
  } catch (error) {
    console.error('❌ Error querying MA DEP Wetlands (nearby):', error);
    return [];
  }
}
