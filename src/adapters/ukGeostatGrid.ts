/**
 * UK GEOSTAT Grid Adapter
 * Queries GEOSTAT Dec 2011 Grid in the United Kingdom polygon feature service
 * Supports point-in-polygon and proximity queries up to 50 miles
 *
 * Service: GEOSTAT_Dec_2011_GEC_in_the_United_Kingdom_2022 (FeatureServer/0)
 ...
 *
 * Fields include: OBJECTID, grd_fixid, grd_floaid, grd_newid, pcd_count,
 * tot_p, tot_m, tot_f, tot_hh, Shape__Area, Shape__Length, GlobalID.
 */

import { fetchJSONSmart } from '../services/EnrichmentService';

export interface UKGeostatGridCellInfo {
  objectId: number;
  grd_fixid: string | null;
  grd_floaid: string | null;
  grd_newid: string | null;
...
function distanceToPolygon(
  ...
  for (const ring of polygon) {
    ...
    for (let i = 0; i < ring.length; i++) {
      ...
      // Coordinates are projected; approximate using input lat/lon for distance
...
export async function getUKGeostatGridData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: UKGeostatGridCellInfo[];
  nearby_features: UKGeostatGridCellInfo[];
  _all: UKGeostatGridCellInfo[];
}> {
  const baseUrl = 'https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/GEOSTAT_Dec_2011_GEC_in_the_United_Kingdom_2022/FeatureServer/0';
  
  const containing: UKGeostatGridCellInfo[] = [];
  const nearby_features: UKGeostatGridCellInfo[] = [];
  const _all: UKGeostatGridCellInfo[] = [];
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        const info: UKGeostatGridCellInfo = {
          objectId: attributes.OBJECTID || attributes.FID || attributes.ESRI_OID || 0,
          grd_fixid: attributes.grd_fixid || attributes.GRD_FIXID || null,
          grd_floaid: attributes.grd_floaid || attributes.GRD_FLOAID || null,
          grd_newid: attributes.grd_newid || attributes.GRD_NEWID || null,
          pcd_count: attributes.pcd_count !== undefined ? Number(attributes.pcd_count) : null,
          tot_p: attributes.tot_p !== undefined ? Number(attributes.tot_p) : null,
          tot_m: attributes.tot_m !== undefined ? Number(attributes.tot_m) : null,
          tot_f: attributes.tot_f !== undefined ? Number(attributes.tot_f) : null,
          tot_hh: attributes.tot_hh !== undefined ? Number(attributes.tot_hh) : null,
