...
export interface DENaturePreserveData {
  containingFeature: DENaturePreserveInfo | null;
  nearbyFeatures: DENaturePreserveInfo[];
}

export interface DENaturePreserveInfo {
  attributes: Record<string, any>;
  geometry?: any;
  distance_miles?: number;
}

async function getContainingFeature(lat: number, lon: number): Promise<DENaturePreserveInfo | null> {
  try {
    ...
    console.error('❌ Error querying DE Nature Preserves:', error);
    return null;
  }
}

async function getNearbyFeatures(lat: number, lon: number, radiusMiles: number): Promise<DENaturePreserveInfo[]> {
  try {
    ...
    const features: DENaturePreserveInfo[] = data.features.map((feature: any) => {
...
export async function getDENaturePreservesData(
  lat: number,
  lon: number,
  radiusMiles: number = 0
): Promise<DENaturePreserveData> {
