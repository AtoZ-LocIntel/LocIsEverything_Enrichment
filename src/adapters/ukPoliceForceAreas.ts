/**
 * UK Police Force Areas Adapter
 * Queries UK Police Force Areas December 2023 polygon feature service
 * Supports point-in-polygon and proximity queries up to 50 miles
 *
 * Service: Police_Force_Areas_December_2023_EW_BFC (FeatureServer/0)
...
export interface UKPoliceForceAreaInfo {
  objectId: number;
  pfa23cd: string | null;
  pfa23nm: string | null;
  bngE: number | null;
...
export async function getUKPoliceForceAreasData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: UKPoliceForceAreaInfo[];
  nearby_features: UKPoliceForceAreaInfo[];
  _all: UKPoliceForceAreaInfo[];
}> {
  const baseUrl =
    'https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/Police_Force_Areas_December_2023_EW_BFC/FeatureServer/0';

  const containing: UKPoliceForceAreaInfo[] = [];
  const nearby_features: UKPoliceForceAreaInfo[] = [];
  const _all: UKPoliceForceAreaInfo[] = [];
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        const info: UKPoliceForceAreaInfo = {
          objectId: attributes.FID || attributes.OBJECTID || attributes.ESRI_OID || 0,
          pfa23cd: attributes.PFA23CD || attributes.pfa23cd || null,
          pfa23nm: attributes.PFA23NM || attributes.pfa23nm || null,
          bngE:
