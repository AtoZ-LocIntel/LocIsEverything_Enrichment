...
export interface IrelandSmallAreaInfo {
  objectId: number;
  guid: string;
  nuts1: string;
  nuts1Name: string;
...
export async function getIrelandSmallAreasData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: IrelandSmallAreaInfo[];
  nearby_features: IrelandSmallAreaInfo[];
  _all: IrelandSmallAreaInfo[];
}> {
  const baseUrl = 'https://services-eu1.arcgis.com/FH5XCsx8rYXqnjF5/ArcGIS/rest/services/Small_Areas_Ungeneralised/FeatureServer/0';
  
  const containing: IrelandSmallAreaInfo[] = [];
  const nearby_features: IrelandSmallAreaInfo[] = [];
  const _all: IrelandSmallAreaInfo[] = [];
  ...
  try {
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        // Extract small area information
        const smallAreaInfo: IrelandSmallAreaInfo = {
          objectId: attributes.OBJECTID_1 || attributes.ESRI_OID || 0,
          guid: attributes.GUID || '',
          nuts1: attributes.NUTS1 || '',
          nuts1Name: attributes.NUTS1NAME || '',
          nuts2: attributes.NUTS2 || '',
          nuts2Name: attributes.NUTS2NAME || '',
          nuts3: attributes.NUTS3 || '',
          nuts3Name: attributes.NUTS3NAME || '',
          county: attributes.COUNTY || '',
          countyName: attributes.COUNTYNAME || '',
          csoEd: attributes.CSOED || '',
          osiEd: attributes.OSIED || '',
          edName: attributes.EDNAME || '',
          saPub2011: attributes.SA_PUB2011 || '',
          smallArea: attributes.SMALL_AREA || '',
          geogId: attributes.GEOGID || '',
          area: attributes.AREA || 0,
          changeCode: attributes.CHANGECODE || 0,
        ...
        if (isContaining) {
          smallAreaInfo.distance_miles = 0;
          containing.push(smallAreaInfo);
        } else {
          ...
          if (distance <= radiusMiles) {
            smallAreaInfo.distance_miles = distance;
            nearby_features.push(smallAreaInfo);
          }
        }
        
        _all.push(smallAreaInfo);
      }
