/**
 * CA Great Gray Owl Range Adapter
 * Queries California Great Gray Owl Range from CDFW BIOS FeatureServer
 * Supports point-in-polygon and proximity queries
 */

const BASE_SERVICE_URL = 'https://services2.arcgis.com/Uq9r85Potqm3MfRV/arcgis/rest/services/biosds898_fpu/FeatureServer';
const LAYER_ID = 0;

export interface CAGreatGrayOwlRangeInfo {
  rangeId: string | null;
...
/**
 * Query CA Great Gray Owl Range FeatureServer for point-in-polygon and proximity
 */
export async function getCAGreatGrayOwlRangeData(
  lat: number,
  lon: number,
  radius?: number
): Promise<CAGreatGrayOwlRangeInfo[]> {
  try {
    const results: CAGreatGrayOwlRangeInfo[] = [];
    
    // Point-in-polygon query
    console.log(`🦉 Querying CA Great Gray Owl Range for containing range at [${lat}, ${lon}]`);
    ...
    if (radius && radius > 0) {
      console.log(`🦉 Querying CA Great Gray Owl Range within ${radius} miles of [${lat}, ${lon}]`);
    ...
    console.log(`✅ Found ${results.length} CA Great Gray Owl Range/Ranges`);
    return results;
  } catch (error) {
    console.error('❌ Error querying CA Great Gray Owl Range:', error);
    return [];
  }
}
