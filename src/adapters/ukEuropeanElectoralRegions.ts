/**
 * UK European Electoral Regions Adapter
 * Queries European Electoral Regions Dec 2018 polygon feature service
 * Supports point-in-polygon and proximity queries up to 25 miles
 *
 * Service: European_Electoral_Regions_Dec_2018_FCB_UK_2022 (FeatureServer/0)
...
export interface UKEuropeanElectoralRegionInfo {
  objectId: number;
  eurg18cd: string | null;
  eurg18nm: string | null;
  bngE: number | null;
  bngN: number | null;
  long: number | null;
  lat: number | null;
...
export async function getUKEuropeanElectoralRegionsData(
  lat: number,
  lon: number,
  radiusMiles: number
): Promise<{
  containing: UKEuropeanElectoralRegionInfo[];
  nearby_features: UKEuropeanElectoralRegionInfo[];
  _all: UKEuropeanElectoralRegionInfo[];
}> {
  const baseUrl =
    'https://services1.arcgis.com/ESMARspQHYMw9BZ9/arcgis/rest/services/European_Electoral_Regions_Dec_2018_FCB_UK_2022/FeatureServer/0';

  const containing: UKEuropeanElectoralRegionInfo[] = [];
  const nearby_features: UKEuropeanElectoralRegionInfo[] = [];
  const _all: UKEuropeanElectoralRegionInfo[] = [];

  try {
    // Cap at 25 miles as requested
    if (radiusMiles > 25.0) radiusMiles = 25.0;
    ...
    while (hasMore) {
      ...
      for (const feature of response.features) {
        ...
        const info: UKEuropeanElectoralRegionInfo = {
          objectId: attributes.FID || attributes.OBJECTID || attributes.ESRI_OID || 0,
          eurg18cd: attributes.EURG18CD || attributes.eurg18cd || null,
          eurg18nm: attributes.EURG18NM || attributes.eurg18nm || null,
          bngE:
