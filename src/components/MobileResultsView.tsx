...
const MobileResultsView: React.FC<MobileResultsViewProps> = ({
  ...
  const formatValue = (value: any, key: string): string => {
    ...
    if (Array.isArray(value)) {
      ...
      if (key.includes('_all_pois') || key.includes('_detailed') || key.includes('_elements') || key.includes('_features') || key.includes('nh_key_destinations_all') || key.includes('nh_nursing_homes_all') || key.includes('nh_ems_all') || key.includes('nh_fire_stations_all') || key.includes('nh_places_of_worship_all') || key.includes('nh_hospitals_all') || key.includes('nh_public_waters_access_all') || key.includes('nh_law_enforcement_all') || key.includes('nh_recreation_trails_all') || key.includes('nh_dot_roads_all') || key.includes('nh_railroads_all') || key.includes('nh_transmission_pipelines_all') || key.includes('nh_cell_towers_all') || key.includes('nh_underground_storage_tanks_all') || key.includes('nh_water_wells_all') || key.includes('nh_public_water_supply_wells_all') || key.includes('nh_remediation_sites_all') || key.includes('nh_des_wetland_permit_applications_all') || key.includes('nh_automobile_salvage_yards_all') || key.includes('nh_solid_waste_facilities_all') || key.includes('nh_nwi_plus_all') || key.includes('ma_dep_wetlands_all') || key.includes('ma_open_space_all') || key.includes('cape_cod_zoning_all') || key.includes('ma_trails_all') || key.includes('ma_nhesp_natural_communities_all') || key.includes('ct_roads_all') || key.includes('ct_building_footprints_all')) {
    ...
    if (key === 'blm_lands_summary_stats' && typeof value === 'object' && value !== null) {
    ...
    if (key === 'blm_pfyc_geologic_formations_summary_stats' && typeof value === 'object' && value !== null) {
      const s = value as Record<string, unknown>;
      const parts: string[] = [];
      if (s.search_radius_miles != null) parts.push(`Search radius: ${s.search_radius_miles} mi`);
      if (s.total_features_returned != null) parts.push(`Total features: ${s.total_features_returned}`);
  ...
  const getEnrichmentCategory = (key: string): string => {
    ...
    if (key.includes('mdb_gtfs_feeds')) {
      return 'Mobility';
    }
    ...
    // City of Miami - check before other state data
    if (key.includes('miami_')) {
    ...
    // Education category - check explicitly for OSM libraries first
    ...
    if (key.includes('poi_town_halls') || key.includes('poi_courthouses') || key.includes('poi_childcare')) {
      return 'Community & Services';
    }
    if (key.includes('poi_parks') || key.includes('poi_tnm_trailheads') || key.includes('poi_tnm_trails') || key.includes('poi_mountain_biking') || key.includes('poi_boat_ramps') || key.includes('poi_golf_courses')) {
    ...
    if (key.includes('poi_police_stations') || key.includes('poi_fire_stations') || key.includes('poi_mail_shipping') || key.includes('poi_post_offices') || key.includes('poi_parcel_lockers') || key.includes('poi_osm_city_town_halls') || key.includes('poi_osm_courthouses') || key.includes('poi_osm_dmv_licensing') || key.includes('poi_osm_zoning_planning') || key.includes('poi_osm_public_works') || key.includes('poi_osm_social_services')) {
    ...
    if (key.includes('poi_bowling') || key.includes('poi_arcade') || key.includes('poi_rv_park') || key.includes('poi_campground')) {
      return 'Other';
    }
    ...
    // Check PADUS and USFS before AT/PCT to avoid false matches (BLM has its own section)
    if (key.includes('padus_') || key.includes('usfs_') || key.includes('nps_') || key.includes('poi_padus_public_access') || key.includes('poi_padus_protection_status')) {
      return 'Public Lands';
    }
  ...
  const groupedEnrichments = Object.entries(enrichments).reduce((acc, [key, value]) => {
    ...
    const isSelectedEnrichment = selectedEnrichments.some(selected => {
      ...
      if (
        key.includes('nh_des_wetland_permit_applications') &&
        key !== 'nh_des_wetland_permit_applications_all'
      ) {
        return selectedEnrichments.includes('nh_des_wetland_permit_applications');
      }
      ...
      if (key.includes('mdb_gtfs_feeds')) {
        return selectedEnrichments.includes('mdb_gtfs_feeds');
      }
