...
const DesktopResultsView: React.FC<DesktopResultsViewProps> = ({
  ...
  const formatValue = (value: any, key: string): string => {
    ...
    if (Array.isArray(value)) {
      ...
      if (key.includes('_all_pois') || key.includes('_detailed') || key.includes('_elements') || key.includes('_features') || key.includes('nh_key_destinations_all') || key.includes('nh_nursing_homes_all') || key.includes('nh_ems_all') || key.includes('nh_fire_stations_all') || key.includes('nh_places_of_worship_all') || key.includes('nh_hospitals_all') || key.includes('nh_public_waters_access_all') || key.includes('nh_law_enforcement_all') || key.includes('nh_recreation_trails_all') || key.includes('nh_dot_roads_all') || key.includes('nh_railroads_all') || key.includes('nh_transmission_pipelines_all') || key.includes('nh_cell_towers_all') || key.includes('nh_underground_storage_tanks_all') || key.includes('nh_water_wells_all') || key.includes('nh_public_water_supply_wells_all') || key.includes('nh_remediation_sites_all') || key.includes('nh_automobile_salvage_yards_all') || key.includes('nh_solid_waste_facilities_all') || key.includes('nh_nwi_plus_all') || key.includes('ma_dep_wetlands_all') || key.includes('ma_open_space_all')) {
  ...
  const groupEnrichments = (enrichments: Record<string, any>) => {
    const grouped = Object.entries(enrichments).reduce((acc, [key, value]) => {
      ...
      } else if (key.includes('ma_dep_wetlands') || key.includes('ma_open_space')) {
