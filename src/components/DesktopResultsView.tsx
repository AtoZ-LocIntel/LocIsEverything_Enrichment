  ...
  const groupEnrichments = (enrichments: Record<string, any>) => {
    const grouped = Object.entries(enrichments).reduce((acc, [key, value]) => {
      ...
      if (key.startsWith('acs_') || key.includes('_acs_')) {
        ...
        category = 'Community & Services';
      } else if (key.includes('poi_') && (key.includes('elementary') || key.includes('school') || key.includes('college') || key.includes('daycares') || key.includes('preschools') || key.includes('vocational') || key.includes('technical') || key.includes('tutoring') || key.includes('library'))) {
        category = 'Education';
